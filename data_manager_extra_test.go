...
func TestCandleRequestKey_DefaultSource(t *testing.T) {
	...
	require.Equal(t, SourceOanda, k.Source)
}
...
func TestRawTickMidOdd(t *testing.T) {
	...
	require.Equal(t, Price(101), tick.Mid())
}
...
func TestStoreScanFiles_ClosedOnlyDailyGapsRemainComplete(t *testing.T) {
	...
	key := Key{Instrument: "EURUSD", Source: SourceOanda, Kind: KindCandle, TF: D1, Year: 2026, Month: 1}
	asset, ok := inv.Get(key)
	require.True(t, ok)
...
func TestWriteMonthlyCandles_WrongCount(t *testing.T) {
	...
	s := newTestStore(t)
	start := time.Date(2026, time.January, 1, 0, 0, 0, 0, time.UTC)
	candles := make([]Candle, 31*24+1)

	err := s.WriteMonthlyCandles(SourceOanda, "EURUSD", H1, start, candles)
	require.Error(t, err)
