...
func TestTradeMargin_Basic(t *testing.T) {
	...
	// EURUSD: quote == USD == account, so quoteToAccount rate == RateScale.
	units := types.Units(10_000)
	price := types.PriceFromFloat(1.1000)
	rate := types.Rate(types.RateScale)

	got, err := broker.TradeMargin(units, price, "EURUSD", rate)
...
func TestTradeMargin_NegativeUnitsSymmetric(t *testing.T) {
	...
	pos, err := broker.TradeMargin(types.Units(1000), price, "EURUSD", rate)
	require.NoError(t, err)
	neg, err := broker.TradeMargin(types.Units(-1000), price, "EURUSD", rate)
	require.NoError(t, err)
...
func TestTradeMargin_ZeroUnits(t *testing.T) {
	...
	got, err := broker.TradeMargin(0, types.PriceFromFloat(1.5), "EURUSD", types.RateScale)
	require.NoError(t, err)
...
func TestTradeMargin_InvalidRate(t *testing.T) {
	...
	_, err := broker.TradeMargin(1000, types.PriceFromFloat(1.1), "EURUSD", 0)
	assert.Error(t, err)
}
...
func TestTradeMargin_InvalidPrice(t *testing.T) {
	...
	_, err := broker.TradeMargin(1000, 0, "EURUSD", types.RateScale)
	assert.Error(t, err)
}
...
func TestTradeMargin_UnknownInstrument(t *testing.T) {
	...
	_, err := broker.TradeMargin(1000, types.PriceFromFloat(1.1), "XXXYYY", types.RateScale)
	assert.Error(t, err)
}
