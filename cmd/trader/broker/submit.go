...
// newSubmitCmd implements "trader broker submit ..." (issue
...
// fills them via
// Broker.Advance, which no CLI command here drives — see New's
...
func runSubmit(cmd *cobra.Command, flags submitFlags) error {
	...
	accountFlags, err := readAccountFlags(cmd)
	if err != nil {
		return err
	}
	accountCfg, err := buildAccountConfig(cmd, accountFlags)
	if err != nil {
		return err
	}
	...
	svc, accountID, err := buildService(cmd, accountCfg, prices)
	if err != nil {
		return err
	}
