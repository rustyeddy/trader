...
func PrintBacktestRun(w io.Writer, r BacktestRunVars) {
	fmt.Fprintln(w, "==================================================")
	...
	fmt.Fprintf(w, "Win Rate:      %.2f%%\n", float64(r.WinRate)*100/float64(rateScale))
	...
	fmt.Fprintf(w, "Return:        %.2f%%\n", float64(r.ReturnPct)*100/float64(rateScale))

	if r.ProfitFactor > 0 {
		fmt.Fprintf(w, "Profit Factor: %.2f\n", float64(r.ProfitFactor)/float64(rateScale))
	}
	if r.MaxDDPct > 0 {
		fmt.Fprintf(w, "Max Drawdown:  %.2f%%\n", float64(r.MaxDDPct)*100/float64(rateScale))
	}
