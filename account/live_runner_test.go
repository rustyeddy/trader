...
func newSeedAccount(t *testing.T, body string) (*Account, func()) {
	...
	acc := NewSession("acc-1", &oanda.Client{BaseURL: srv.URL, Token: "test"}, nil)
...
func newPricingAccount(t *testing.T, srv *httptest.Server) *Account {
	t.Helper()
	return NewSession("acc-1", &oanda.Client{BaseURL: srv.URL, Token: "tok", HTTP: srv.Client()}, nil)
}
