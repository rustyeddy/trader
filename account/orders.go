...
func (acct *Account) PlaceMarketOrder(ctx context.Context, req PlaceMarketOrderRequest) (*PlaceMarketOrderResult, error) {
	side := strings.ToLower(strings.TrimSpace(req.Side))
	if side != "long" && side != "short" {
		return nil, fmt.Errorf("side must be 'long' or 'short', got %q", req.Side)
	}

	prices, err := acct.OANDA.GetPricing(ctx, acct.ID, req.Instrument)
	if err != nil {
	...
	if snap := acct.getSnapshot(); snap != nil {
		...
		s, err := acct.broker().GetAccountSummary(ctx, acct.ID)
		if err != nil {
	...
	if req.Units != 0 {
		...
		if err := SizePosition(inputs, openReq); err != nil {
	...
	fill, err := acct.broker().SubmitMarketOrder(ctx, acct.ID, req.Instrument, units, stopPrice.Float64())
	if err != nil {
		return result, fmt.Errorf("submit order: %w", err)
	}
	result.Filled = fill
	if acct.Log != nil {
		acct.Log.Info("account: market order filled",
...
func (acct *Account) CloseTrade(ctx context.Context, tradeID string, units int64) (*oanda.CloseTradeResult, error) {
	res, err := acct.broker().CloseTrade(ctx, acct.ID, tradeID, units)
	if err != nil {
		return nil, fmt.Errorf("close trade %s: %w", tradeID, err)
	}
	if acct.Log != nil {
		acct.Log.Info("account: trade closed",
...
func (acct *Account) UpdateTradeStop(ctx context.Context, tradeID string, stopPx, takePx float64) error {
	if err := acct.broker().UpdateTradeStop(ctx, acct.ID, tradeID, stopPx, takePx); err != nil {
...
func (acct *Account) ListOpenTrades(ctx context.Context) ([]oanda.OpenTrade, error) {
	if snap := acct.getSnapshot(); snap != nil {
		return snap.OpenTrades(), nil
	}
	trades, err := acct.broker().GetOpenTrades(ctx, acct.ID)
	if err != nil {
