...
func TestTradeMargin_Basic(t *testing.T) {
	...
	got, err := account.TradeMargin(units, price, "EURUSD", rate)
...
func TestTradeMargin_NegativeUnitsSymmetric(t *testing.T) {
	...
	pos, err := account.TradeMargin(types.Units(1000), price, "EURUSD", rate)
	require.NoError(t, err)
	neg, err := account.TradeMargin(types.Units(-1000), price, "EURUSD", rate)
	require.NoError(t, err)
...
func TestTradeMargin_InvalidRate(t *testing.T) {
	...
	_, err := account.TradeMargin(1000, types.PriceFromFloat(1.1), "EURUSD", 0)
	assert.Error(t, err)
}
