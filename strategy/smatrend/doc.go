...
// smatrend's own responsibility at all: the resting order.
// IntentAdjustStop it places is triggered by the real broker-side
// machinery (ADR-026, wired into backtest.Scheduler by issue #338), so
// by the time OnBar observes a bar, any stop hit against that bar's own
// price action has already closed the position in the account snapshot
// smatrend reads through strategy.View. smatrend's own job is only:
// track the high-water mark and the monotonic (never-decreasing) stop
// level while long, and require a fresh SMA cross-above (not merely
// "still above the SMA") before re-entering after a stop exit.
package smatrend
