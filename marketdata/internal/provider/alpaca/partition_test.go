...
func TestOpen_ReadsRecordsAndMeta(t *testing.T) {
	...
	path := writePartitionFile(t, root, "SPY", 2020, time.May,
		"# schema=raw-v1 source=alpaca instrument=SPY tf=d1 year=2020 month=05\n"+
...
func TestReader_SchemaCrossCheckMismatches(t *testing.T) {
	...
	cases := map[string]string{
		"wrong schema":     "# schema=raw-v2 source=alpaca instrument=SPY tf=d1 year=2020 month=05\n",
		"wrong source":     "# schema=raw-v1 source=oanda instrument=SPY tf=d1 year=2020 month=05\n",
		"wrong instrument": "# schema=raw-v1 source=alpaca instrument=QQQ tf=d1 year=2020 month=05\n",
		"wrong tf":         "# schema=raw-v1 source=alpaca instrument=SPY tf=h1 year=2020 month=05\n",
		"wrong year":       "# schema=raw-v1 source=alpaca instrument=SPY tf=d1 year=2021 month=05\n",
		"wrong month":      "# schema=raw-v1 source=alpaca instrument=SPY tf=d1 year=2020 month=06\n",
	}
...
func TestReadPartitionRecords_ReturnsRecordsInFileOrder(t *testing.T) {
	root := t.TempDir()
	ctx := context.Background()
	recs := []Record{
	...
	require.NoError(t, err)
	require.Len(t, got, 2)
	assert.Equal(t, "100", got[0].Open.String())
	assert.Equal(t, "101", got[1].Open.String())
}

func TestReadPartitionRecords_MissingFile(t *testing.T) {
	_, err := ReadPartitionRecords(context.Background(), t.TempDir(), "SPY", 2020, time.May)
	assert.True(t, errors.Is(err, os.ErrNotExist))
}

func TestReadPartitionRecords_CancelledContext(t *testing.T) {
	root := t.TempDir()
	ctx := context.Background()
	rec := Record{Time: time.Date(2020, 5, 1, 0, 0, 0, 0, time.UTC)}
	require.NoError(t, WritePartition(ctx, root, "SPY", 2020, time.May, []Record{rec}, true))
	...
	cancel()
	_, err := ReadPartitionRecords(cancelled, root, "SPY", 2020, time.May)
