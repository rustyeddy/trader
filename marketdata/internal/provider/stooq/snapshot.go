...
// oanda.PartitionSnapshot's own counterpart.
...
// that same in-memory byte slice, so Records and Fingerprint necessarily
// describe one identical revision of the file — the same one-read
// guarantee oanda.ReadPartitionSnapshot provides, for the identical
// reason (marketdata's canonical normalization build needs a raw
// fingerprint that describes the exact data it built from).
