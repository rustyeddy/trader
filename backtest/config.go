...
func (r ResolvedRun) CandleRequest() (data.CandleRequest, error) {
	tf, err := parseTimeframe(r.Timeframe)
	if err != nil {
		return data.CandleRequest{}, err
	}

	start, err := parseDateStart(r.From)
	if err != nil {
		return data.CandleRequest{}, fmt.Errorf("bad from %q: %w", r.From, err)
	}

	end, err := parseDateEndExclusive(r.To)
	if err != nil {
		return data.CandleRequest{}, fmt.Errorf("bad to %q: %w", r.To, err)
	}

	return data.CandleRequest{
		Source:     r.Source,
