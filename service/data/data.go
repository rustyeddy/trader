// Package datasvc is the service-layer orchestration for candle
// acquisition/derivation/validation/stats/export, mirroring cmd/data. It is
// protocol-agnostic (see service/service.go's package doc) — shared by
// cmd/data, api/rest, and api/mcp, which reach it via the aliases
// re-exported from the top-level service package (see service/data.go)
// rather than importing this package directly.
//
// Named datasvc, not data, purely for naming symmetry with the other
// service/<domain> packages split out of service/ (backtestsvc, reviewsvc)
// — no top-level "data" package exists to actually collide with, but the
// consistent suffix keeps the convention obvious at a glance.
package datasvc
