...
func TestPosUnitsForNotional_USDBase(t *testing.T) {
	...
	assert.Equal(t, int64(1000), posUnitsForNotional(inst, types.PriceFromFloat(150.0), 1000.0))
	assert.Equal(t, int64(501), posUnitsForNotional(inst, types.PriceFromFloat(150.0), 500.5)) // math.Round rounds half away from zero
}
...
func TestPosUnitsForNotional_NonUSDBase(t *testing.T) {
	...
	got := posUnitsForNotional(inst, types.PriceFromFloat(1.10), 10_000.0)
