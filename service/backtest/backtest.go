// Package backtestsvc is the service-layer orchestration for running and
// persisting backtests, mirroring cmd/backtest. It is protocol-agnostic
// (see service/service.go's package doc) — shared by cmd/backtest,
// api/rest, and api/mcp, which reach it via the aliases re-exported from
// the top-level service package rather than importing this package
// directly (see service/backtest.go).
//
// Named backtestsvc, not backtest, because the top-level
// "github.com/rustyeddy/trader/backtest" package (compiled backtest
// definitions/execution) is imported alongside this one in every real call
// site; two packages both named backtest in the same file would collide.
package backtestsvc
...
type Service struct {
	...
	// TraderBacktestExecutor.
	Executor backtest.BacktestExecutor
	Log      *slog.Logger
}
