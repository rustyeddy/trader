...
type IntentFactory interface {
	...
	Enter(instID instrument.ID, side order.Side) (runtimeorder.Intent, error)
	// Exit builds an order.IntentExit for instID.
	Exit(instID instrument.ID) (runtimeorder.Intent, error)
	// AdjustStop builds an order.IntentAdjustStop moving instID's
	// protective stop to stopPrice.
	AdjustStop(instID instrument.ID, stopPrice num.Price) (runtimeorder.Intent, error)
	...
	EnterWithStop(instID instrument.ID, side order.Side, stopPrice num.Price) (runtimeorder.Intent, error)
	// TargetExposure builds an order.IntentTargetExposure for instID,
	// side, and quantity.
	TargetExposure(instID instrument.ID, side order.Side, quantity num.Quantity) (runtimeorder.Intent, error)
...
func (f *intentFactory) Enter(instID instrument.ID, side order.Side) (runtimeorder.Intent, error) {
	return f.build(order.IntentEnter, instID, side, nil, nil)
}

func (f *intentFactory) Exit(instID instrument.ID) (runtimeorder.Intent, error) {
	return f.build(order.IntentExit, instID, 0, nil, nil)
}

func (f *intentFactory) AdjustStop(instID instrument.ID, stopPrice num.Price) (runtimeorder.Intent, error) {
	return f.build(order.IntentAdjustStop, instID, 0, nil, &stopPrice)
}

func (f *intentFactory) EnterWithStop(instID instrument.ID, side order.Side, stopPrice num.Price) (runtimeorder.Intent, error) {
	return f.build(order.IntentEnterWithStop, instID, side, nil, &stopPrice)
}

func (f *intentFactory) TargetExposure(instID instrument.ID, side order.Side, quantity num.Quantity) (runtimeorder.Intent, error) {
...
func (f *intentFactory) build(kind order.IntentKind, instID instrument.ID, side order.Side, quantity *num.Quantity, stopPrice *num.Price) (runtimeorder.Intent, error) {
