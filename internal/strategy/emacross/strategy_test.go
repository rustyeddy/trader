...
func mustSnapshot(t *testing.T, accountID id.AccountID, position *runtimeorder.Position) account.Snapshot {
	t.Helper()
...
func (h *testHarness) buildBar(barNum int, close float64) (strategy.BarEvent, strategy.View, time.Time) {
	...
	var position *runtimeorder.Position
...
func (h *testHarness) onBar(barNum int, close float64) ([]runtimeorder.Intent, time.Time) {
	h.t.Helper()
...
func TestStrategy_DeterministicAcrossRepeatedRuns(t *testing.T) {
	run := func() []runtimeorder.Intent {
		h := newTestHarness(t, Config{FastPeriod: 3, SlowPeriod: 5})
		var all []runtimeorder.Intent
		for i, close := range emaFixtureCloses {
			intents, _ := h.onBar(i+1, close)
...
func (f *erroringIntentFactory) Enter(instID instrument.ID, side order.Side) (runtimeorder.Intent, error) {
	if f.failEnter {
		return runtimeorder.Intent{}, errBoom
	}
	return f.IntentFactory.Enter(instID, side)
}

func (f *erroringIntentFactory) Exit(instID instrument.ID) (runtimeorder.Intent, error) {
	if f.failExit {
		return runtimeorder.Intent{}, errBoom
	}
