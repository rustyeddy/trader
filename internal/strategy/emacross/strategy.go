...
func (s *Strategy) actOnCross(side order.PositionSide, want order.Side) (string, []runtimeorder.Intent, error) {
	...
	case order.Flat:
		...
		if wantsLong {
			return "enter-long", []runtimeorder.Intent{in}, nil
		}
		return "enter-short", []runtimeorder.Intent{in}, nil
...
func (s *Strategy) exitOnly() (string, []runtimeorder.Intent, error) {
	in, err := s.intents.Exit(s.instrumentID)
	if err != nil {
		return "none", nil, err
	}
	return "exit", []runtimeorder.Intent{in}, nil
}
...
// correlate with.
func (s *Strategy) recordSignal(ctx context.Context, event strategy.BarEvent, prevRelation relation, havePrev bool, current relation, cross, action string, intents []runtimeorder.Intent) error {
	if s.journal == nil {
		return nil
	}
...
func (s *Strategy) reverse(want order.Side) ([]runtimeorder.Intent, error) {
	...
	return []runtimeorder.Intent{exit, enter}, nil
}
