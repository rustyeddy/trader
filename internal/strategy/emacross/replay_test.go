...
func (r *recordingStrategy) OnBar(ctx context.Context, event strategy.BarEvent, view strategy.View) ([]runtimeorder.Intent, error) {
...
func TestEmacross_RunsThroughCanonicalMarketdataReplay(t *testing.T) {
	...
	reader, err := manager.Bars(ctx, marketruntime.BarQuery{Instrument: simListing.InstrumentID(), Interval: marketdata.H1, Range: span})
	require.NoError(t, err)
...
func TestEmacross_UncoveredRangeFailsClearly(t *testing.T) {
	...
	ctx := context.Background()
	query := marketruntime.BarQuery{Instrument: instID, Interval: marketdata.H1, Range: span}
	...
	assert.Equal(t, marketruntime.PartitionCoverageMissing, plan.Coverage.Partitions[0].Status)
	require.Len(t, plan.Actions, 1)
	assert.Equal(t, marketruntime.ActionDownloadRaw, plan.Actions[0].Kind)
