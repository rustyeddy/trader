...
func loadBarLookupPriceSource(t *testing.T, ctx context.Context, manager *marketruntime.Manager, c *clock.Simulated, symbol string, query marketruntime.BarQuery) *barLookupPriceSource {
	t.Helper()
...
func runEMACrossoverFixtureWithStrategyConfig(t *testing.T, cfg emacross.Config, rules ...risk.Rule) (svcbacktest.RunResponse, *memoryRecorder) {
	...
	query := marketruntime.BarQuery{Instrument: simListing.InstrumentID(), Interval: marketdata.H1, Range: span}
