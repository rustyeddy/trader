...
func (s *Strategy) OnBar(ctx context.Context, event strategy.BarEvent, view strategy.View) ([]runtimeorder.Intent, error) {
...
func (s *Strategy) onFlat(ctx context.Context, event strategy.BarEvent, crossedAbove, aboveSMA bool, close, smaValue float64) ([]runtimeorder.Intent, error) {
	var enter bool
	switch {
	case !s.everExited:
	...
	if err := s.recordSignal(ctx, event, close, smaValue, PhaseFlat, action, initialStop, []runtimeorder.Intent{in}); err != nil {
		return nil, err
	}
...
func (s *Strategy) buildEntryIntent(smaValue float64) (runtimeorder.Intent, *num.Price, error) {
	provider, ok := s.exitRule.(InitialStopProvider)
	if !ok {
		in, err := s.intents.Enter(s.instrumentID, order.Buy)
		if err != nil {
			return runtimeorder.Intent{}, nil, err
		}
		s.pendingInitialStop = nil
		return in, nil, nil
	}
	...
	if err != nil {
		return runtimeorder.Intent{}, nil, fmt.Errorf("smatrend: computing initial stop for bracket entry: %w", err)
	}
...
func (s *Strategy) onLong(ctx context.Context, event strategy.BarEvent, close, smaValue float64) ([]runtimeorder.Intent, error) {
	...
	if decision.ExitNow {
		...
		if err := s.recordSignal(ctx, event, close, smaValue, phase, "exit-now", nil, []runtimeorder.Intent{in}); err != nil {
			return nil, err
		}
		return []runtimeorder.Intent{in}, nil
	}
	...
	if err := s.recordSignal(ctx, event, close, smaValue, phase, "adjust-stop", decision.NewStop, []runtimeorder.Intent{in}); err != nil {
		return nil, err
	}
	return []runtimeorder.Intent{in}, nil
}
...
func (s *Strategy) recordSignal(ctx context.Context, event strategy.BarEvent, close, smaValue float64, phase Phase, action string, stopPrice *num.Price, intents []runtimeorder.Intent) error {
	if s.journal == nil {
		return nil
	}
