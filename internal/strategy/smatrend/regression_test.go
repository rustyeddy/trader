...
func loadBarLookupPriceSource(t *testing.T, ctx context.Context, manager *marketruntime.Manager, c *clock.Simulated, query marketruntime.BarQuery) *barLookupPriceSource {
	t.Helper()
...
func runSMATrendFixtureForSpan(t *testing.T, cfg smatrend.Config, span marketdata.TimeRange) (svcbacktest.RunResponse, *memoryRecorder) {
	...
	query := marketruntime.BarQuery{Instrument: simListing.InstrumentID(), Interval: marketdata.D1, Range: span}
