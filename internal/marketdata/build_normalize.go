...
func (m *Manager) normalizeAndPublish(ctx context.Context, action Action) (PublishResult, error) {
	...
	bs := marketdata.BarSet{Instrument: action.Instrument, Interval: action.Interval, Span: span, Basis: basis, Bars: bars}
...
func (m *Manager) readAndNormalizeRaw(ctx context.Context, rawInterval, symbol string, action Action) ([]normalizedRecord, string, marketdata.PriceBasis, marketdata.AdjustmentPolicy, string, string, error) {
	...
	case "stooq":
		if rawInterval != string(stooq.RawD1) {
			return nil, "", marketdata.BasisUnknown, marketdata.AdjustmentUnknown, "", "", fmt.Errorf("marketdata: stooq: only %s is supported, got %s", marketdata.D1, action.Interval)
		}
		...
		if _, ok := m.calendar.(*marketdata.USEquityCalendar); !ok {
			return nil, "", marketdata.BasisUnknown, marketdata.AdjustmentUnknown, "", "", fmt.Errorf(
		...
		if err != nil {
			return nil, "", marketdata.BasisUnknown, marketdata.AdjustmentUnknown, "", "", fmt.Errorf("read raw partition: %w", err)
		}
		if err := ctx.Err(); err != nil {
			return nil, "", marketdata.BasisUnknown, marketdata.AdjustmentUnknown, "", "", err
		}
	...
	case "alpaca":
		if rawInterval != string(alpaca.RawD1) {
			return nil, "", marketdata.BasisUnknown, marketdata.AdjustmentUnknown, "", "", fmt.Errorf("marketdata: alpaca: only %s is supported, got %s", marketdata.D1, action.Interval)
		}
