...
func downloadAction(year int, month time.Month) Action {
	return Action{
		Kind: ActionDownloadRaw, Instrument: eurusd(), Interval: marketdata.H1,
...
func TestSync_ExtendUsesActualLastRecordEvenIfFileOutOfOrder(t *testing.T) {
	...
	writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2020, time.March,
...
func TestSync_RefetchesIncompleteTailRecordInstead(t *testing.T) {
	...
	writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2020, time.March, rawRow(tailTime, false))
...
func TestSync_SkipsNonDownloadActions(t *testing.T) {
	...
	plan := Plan{Actions: []Action{
		{Kind: ActionNormalizeCanonical, Instrument: eurusd(), Interval: marketdata.H1, Year: 2020, Month: time.March},
		{Kind: ActionDeriveCanonical, Instrument: eurusd(), Interval: marketdata.W1, Year: 2020, Month: time.March},
	}}
	result, err := mgr.Sync(context.Background(), plan)
	require.NoError(t, err)
...
func TestSync_AlwaysSkipsRepairRaw(t *testing.T) {
	...
	plan := Plan{Actions: []Action{{
		Kind: ActionRepairRaw, Instrument: eurusd(), Interval: marketdata.H1,
