...
func TestNormalizeOANDARecord_Suspicious(t *testing.T) {
	...
	assert.Equal(t, marketdata.Bar{}, nr.bar, "no candidate Bar for a suspicious record")
...
func TestNormalizeOANDARecord_RejectedBadOHLC(t *testing.T) {
	...
	assert.ErrorIs(t, nr.err, marketdata.ErrBarOHLC)
}
...
func TestNormalizeOANDARecord_RejectedNegativeTicks(t *testing.T) {
	...
	assert.Equal(t, recordOutcomeRejected, nr.outcome)
	assert.ErrorIs(t, nr.err, marketdata.ErrBarTicks)
}
...
func TestCanonicalInterval(t *testing.T) {
	cases := []struct {
		...
		want marketdata.Interval
	}{
		{oanda.RawM1, marketdata.M1},
		{oanda.RawH1, marketdata.H1},
		{oanda.RawH4, marketdata.H4},
		{oanda.RawD1, marketdata.D1},
	}
...
func h1FXCalendar() *marketdata.FXCalendar {
	return marketdata.NewFXCalendar(marketdata.FXCalendarParams{})
}

func TestNormalizeOANDASequence_NilCalendar(t *testing.T) {
	_, err := normalizeOANDASequence(oanda.RawH1, nil, nil)
	assert.ErrorIs(t, err, marketdata.ErrNilCalendar)
}
...
func TestNormalizeOANDASequence_D1AcrossSpringForward(t *testing.T) {
	cal := h1FXCalendar()
	span, err := cal.Bar(nyTime(2026, time.March, 8, 10, 0), marketdata.D1)
	require.NoError(t, err)
...
func (failingCalendar) Status(time.Time) marketdata.Status { return marketdata.StatusOpen }
func (failingCalendar) Session(time.Time) (marketdata.TimeRange, bool) {
	return marketdata.TimeRange{}, true
}
func (c failingCalendar) Bar(time.Time, marketdata.Interval) (marketdata.TimeRange, error) {
	return marketdata.TimeRange{}, c.err
}

var _ marketdata.Calendar = failingCalendar{}
