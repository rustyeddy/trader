...
// apples to apples.
func nativeBarsFromRaw(cal marketdata.Calendar, rawInterval oanda.RawInterval, records []oanda.Record) ([]marketdata.Bar, error) {
...
func groupBarsByInterval(cal marketdata.Calendar, bars []marketdata.Bar, interval marketdata.Interval) (map[int64][]marketdata.Bar, error) {
	groups := make(map[int64][]marketdata.Bar)
	for _, b := range bars {
...
func compareBars(symbol string, year int, month time.Month, derived, native []marketdata.Bar, tolerance num.Price) []disagreement {
	derivedByTime := make(map[int64]marketdata.Bar, len(derived))
	for _, b := range derived {
		derivedByTime[b.Time.UTC().UnixNano()] = b
	}
	nativeByTime := make(map[int64]marketdata.Bar, len(native))
...
// at here.
func runCorpusComparison(t *testing.T, sourceRawInterval, nativeRawInterval oanda.RawInterval, targetInterval marketdata.Interval) {
	t.Helper()
	if corpusRawRoot == "" {
	...
	cal := marketdata.NewFXCalendar(marketdata.FXCalendarParams{})
	...
	for _, p := range inv.Partitions {
		...
		var derivedBars []marketdata.Bar
...
func TestCorpusH4AgreesWithNative(t *testing.T) {
	runCorpusComparison(t, oanda.RawH1, oanda.RawH4, marketdata.
...
func TestCorpusD1AgreesWithNativeFromH1(t *testing.T) {
	runCorpusComparison(t, oanda.RawH1, oanda.RawD1, marketdata.
...
func TestCorpusD1AgreesWithNativeFromH4(t *testing.T) {
	runCorpusComparison(t, oanda.RawH4, oanda.RawD1, marketdata.
...
func TestCorpusLegacyComparison(t *testing.T) {
	...
	for _, p := range inv.Partitions {
		...
		newByTime := make(map[int64]marketdata.Bar, len(newBars))
