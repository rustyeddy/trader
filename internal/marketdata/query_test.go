...
// scope is read-only).
func publishTestPartition(t *testing.T, mgr *Manager, key partitionKey, m marketdata.Manifest, bs marketdata.BarSet) {
	t.Helper()
...
func TestManagerBars_NarrowerRangeFiltersBars(t *testing.T) {
	...
	narrow, err := marketdata.NewTimeRange(m.Span.Start(), m.Span.Start().Add(time.Hour))
	require.NoError(t, err)

	reader, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: narrow})
	require.NoError(t, err)
...
func TestManagerBars_SpansMultipleMonths(t *testing.T) {
	...
	marchSpan, err := marketdata.NewTimeRange(marchManifest.Span.Start(), aprilStart)
	require.NoError(t, err)
	marchManifest.Span = marchSpan
	marchBars.Span = marchSpan
	publishTestPartition(t, mgr, marchKey, marchManifest, marchBars)

	aprilSpan, err := marketdata.NewTimeRange(aprilStart, aprilEnd)
	require.NoError(t, err)
	aprilBars := marchBars
	aprilBars.Span = aprilSpan
	aprilBars.Bars = []marketdata.Bar{barAt(t, aprilStart)}
	...
	full, err := marketdata.NewTimeRange(marchManifest.Span.Start(), aprilEnd)
	require.NoError(t, err)
	reader, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: full})
	require.NoError(t, err)
...
func TestManagerBars_GapInCoverageReportsErrDataUnavailable(t *testing.T) {
	...
	wide, err := marketdata.NewTimeRange(m.Span.Start(), m.Span.End().Add(20*time.Hour))
	require.NoError(t, err)

	_, err = mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: wide})
...
func TestManagerBars_BoundarySpilloverIsFoundViaProbe(t *testing.T) {
	...
	bs := marketdata.BarSet{Instrument: eurusd(), Interval: marketdata.D1, Span: span, Basis: marketdata.BasisBid, Bars: []marketdata.Bar{bar1, bar2}}
	require.NoError(t, bs.Validate())

	man := validManifest(t)
	man.Interval = marketdata.D1
	man.Span = span
	man.BarCount = 2
	...
	queryRange, err := marketdata.NewTimeRange(spanStart, time.Date(2020, 3, 31, 18, 0, 0, 0, time.UTC))
	require.NoError(t, err)

	reader, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.D1, Range: queryRange})
	require.NoError(t, err)
...
func publishOverlappingMonthPair(t *testing.T, mgr *Manager, marchBar, aprilBar marketdata.Bar) (queryRange marketdata.TimeRange) {
	t.Helper()

	marchSpan, err := marketdata.NewTimeRange(
		time.Date(2020, 3, 31, 23, 0, 0, 0, time.UTC),
		time.Date(2020, 4, 1, 1, 0, 0, 0, time.UTC),
	)
	require.NoError(t, err)
	marchKey := partitionKey{provider: "oanda", symbol: "EURUSD", instrument: eurusd(), interval: marketdata.H1, year: 2020, month: time.March}
	marchBS := marketdata.BarSet{Instrument: eurusd(), Interval: marketdata.H1, Span: marchSpan, Basis: marketdata.BasisBid, Bars: []marketdata.Bar{marchBar}}
	...
	aprilSpan, err := marketdata.NewTimeRange(
	...
	aprilKey := marchKey
	aprilKey.month = time.April
	aprilBS := marketdata.BarSet{Instrument: eurusd(), Interval: marketdata.H1, Span: aprilSpan, Basis: marketdata.BasisBid, Bars: []marketdata.Bar{aprilBar}}
	...
	full, err := marketdata.NewTimeRange(marchSpan.Start(), aprilSpan.End())
	require.NoError(t, err)
	return full
}
...
func TestManagerBars_IdenticalOverlapDuplicateIsAccepted(t *testing.T) {
	...
	reader, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: queryRange})
	require.NoError(t, err)
...
func TestManagerBars_ConflictingOverlapDuplicateReportsError(t *testing.T) {
	...
	_, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: queryRange})
...
func TestManagerBars_MissingPartitionReportsErrDataUnavailable(t *testing.T) {
	...
	_, err = mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
	assert.ErrorIs(t, err, ErrDataUnavailable)
}
...
func TestManagerBars_InvalidQuery(t *testing.T) {
	...
	cases := map[string]BarQuery{
		"ZeroInstrument":  {Instrument: instrument.ID{}, Interval: marketdata.H1, Range: span},
		"InvalidInterval": {Instrument: eurusd(), Interval: marketdata.Interval{}, Range: span},
		"ZeroRange":       {Instrument: eurusd(), Interval: marketdata.H1, Range: marketdata.TimeRange{}},
	}
...
func TestManagerBars_UnresolvedInstrumentErrors(t *testing.T) {
	...
	_, err = mgr.Bars(context.Background(), BarQuery{Instrument: gbpusd(), Interval: marketdata.H1, Range: span})
	assert.Error(t, err)
...
func TestManagerBars_CancelledContext(t *testing.T) {
	...
	_, err := mgr.Bars(ctx, BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: validManifest(t).Span})
	assert.ErrorIs(t, err, context.Canceled)
}

func TestManagerBars_NotConfiguredReportsError(t *testing.T) {
	var mgr Manager
	_, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: validManifest(t).Span})
	assert.ErrorIs(t, err, ErrInvalidConfig)

	var nilMgr *Manager
	_, err = nilMgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: validManifest(t).Span})
	assert.ErrorIs(t, err, ErrInvalidConfig)
}
...
func TestManagerBars_ManifestMutationDoesNotPoisonSubsequentQuery(t *testing.T) {
	...
	reader1, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: m.Span})
	require.NoError(t, err)
	...
	reader2, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: m.Span})
	require.NoError(t, err)
...
func TestBarQueryValidate(t *testing.T) {
	span, err := marketdata.NewTimeRange(time.Now(), time.Now().Add(time.Hour))
	require.NoError(t, err)
	valid := BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span}
...
func TestMonthPartitionKeys_SingleMonth(t *testing.T) {
	...
	require.NoError(t, err)
	keys := monthPartitionKeys("oanda", "EURUSD", eurusd(), marketdata.H1, span)
...
func TestCoverageGap_FullyCovered(t *testing.T) {
	want, err := marketdata.NewTimeRange(
		time.Date(2020, 3, 1, 0, 0, 0, 0, time.UTC),
		time.Date(2020, 3, 2, 0, 0, 0, 0, time.UTC),
	)
	require.NoError(t, err)
	gap, ok := coverageGap(want, []marketdata.TimeRange{want})
	assert.True(t, ok)
	assert.Equal(t, marketdata.TimeRange{}, gap)
}
...
func TestCoverageGap_GapInMiddleDetected(t *testing.T) {
	...
	require.NoError(t, err)
	first, err := marketdata.NewTimeRange(
		time.Date(2020, 3, 1, 0, 0, 0, 0, time.UTC),
		time.Date(2020, 3, 2, 0, 0, 0, 0, time.UTC),
	)
	require.NoError(t, err)
	last, err := marketdata.NewTimeRange(
	...
	gap, ok := coverageGap(want, []marketdata.TimeRange{last, first})
	require.False(t, ok)
...
func TestCoverageGap_OverlappingSpansMergeCleanly(t *testing.T) {
	...
	require.NoError(t, err)
	a, err := marketdata.NewTimeRange(
		time.Date(2020, 3, 1, 0, 0, 0, 0, time.UTC),
		time.Date(2020, 3, 2, 12, 0, 0, 0, time.UTC),
	)
	require.NoError(t, err)
	b, err := marketdata.NewTimeRange(
		time.Date(2020, 3, 2, 0, 0, 0, 0, time.UTC),
		time.Date(2020, 3, 3, 0, 0, 0, 0, time.UTC),
	)
	require.NoError(t, err)

	_, ok := coverageGap(want, []marketdata.TimeRange{a, b})
	assert.True(t, ok)
}
