...
func (f fakeStore) publish(context.Context, partitionKey, marketdata.Manifest, marketdata.BarSet) error {
...
func TestNewManagerDefaultsCalendarWhenUnset(t *testing.T) {
	...
	if _, ok := m.calendar.(*marketdata.FXCalendar); !ok {
		t.Fatalf("expected *FXCalendar default, got %T", m.calendar)
	}
}

func TestNewManagerHonorsExplicitCalendar(t *testing.T) {
	cal := marketdata.NewFXCalendar(marketdata.FXCalendarParams{Holidays: []time.Time{time.Date(2024, 12, 25, 0, 0, 0, 0, time.UTC)}})
	m, err := New(Config{
		Clock:        testClock(),
		StoreRoot:    "/data/candles",
		Resolver:     testResolver(t),
