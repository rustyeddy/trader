...
func TestPlan_DownloadMissingRawGatesNormalize(t *testing.T) {
	...
	plan, err := mgr.Plan(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
	require.NoError(t, err)
...
func TestPlan_NormalizeStaleCanonical(t *testing.T) {
	...
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, validRawFingerprint, nil) // deliberately wrong fingerprint
...
func TestPlan_ExtendsWhenLastRawRecordIsIncomplete(t *testing.T) {
	...
	rawPath := writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2024, time.January, rawRow(start, false))
	fp := rawFingerprint(t, rawPath)
	bars := []marketdata.Bar{barAt(t, start)}
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, fp, nil)
...
func TestPlan_ExtendCurrentMonthWhenRawLagsCalendar(t *testing.T) {
	...
	span, err := marketdata.NewTimeRange(aWeekday(0), now)
	require.NoError(t, err)
	rawPath := writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2024, time.January, rawRow(aWeekday(0), true))
	fp := rawFingerprint(t, rawPath)
	bars := []marketdata.Bar{barAt(t, aWeekday(0))}
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, fp, nil)
...
func TestPlan_DeriveW1GatedOnD1Completeness(t *testing.T) {
	...
	plan, err := mgr.Plan(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: span})
	require.NoError(t, err)
	...
	dayBoundary, err := mgr.calendar.Bar(aWeekday(0), marketdata.D1)
	require.NoError(t, err)
	d1Bars := []marketdata.Bar{barAt(t, dayBoundary.Start())}
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, dayBoundary, d1Bars, validRawFingerprint, nil)

	plan2, err := mgr.Plan(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: span})
	require.NoError(t, err)
	derives := actionsOfKind(plan2.Actions, ActionDeriveCanonical)
	require.Len(t, derives, 1)
	assert.Equal(t, "missing", derives[0].Reason)
	assert.Equal(t, marketdata.W1, derives[0].Interval)
}
...
func TestPlan_CancelledContext(t *testing.T) {
	...
	_, err = mgr.Plan(ctx, BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
...
func TestPlan_NotConfigured(t *testing.T) {
	...
	span, err := marketdata.NewTimeRange(aWeekday(0), aWeekday(1))
	require.NoError(t, err)
	_, err = mgr.Plan(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
...
func TestPlan_W1ConvergesAfterBoundaryGapFillsIn(t *testing.T) {
	...
	janFirstDay, err := firstBoundaryAtOrAfter(mgr.calendar, janSpan.Start(), marketdata.D1)
	require.NoError(t, err)
	janDaySpan, err := marketdata.NewTimeRange(janFirstDay, janSpan.End())
	require.NoError(t, err)
	allJanD1Bars := fullWeekD1Bars(t, mgr, janDaySpan)
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, janSpan, allJanD1Bars, validRawFingerprint, nil)
	...
	result1, err := mgr.Build(context.Background(), Plan{Actions: []Action{
		{Kind: ActionDeriveCanonical, Instrument: eurusd(), Interval: marketdata.W1, Year: 2024, Month: time.January},
	}})
	require.NoError(t, err)
	...
	cov1, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: janSpan})
	require.NoError(t, err)
	...
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.February, deriveMonthSpan(t, 2024, time.February), boundaryFebBars, validRawFingerprint, nil)
	...
	plan2, err := mgr.Plan(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: janSpan})
	require.NoError(t, err)
	derives := actionsOfKind(plan2.Actions, ActionDeriveCanonical)
	...
	cov2, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: janSpan})
	require.NoError(t, err)
	...
	narrowRange, err := marketdata.NewTimeRange(boundaryWeek.Start(), boundaryWeek.Start().Add(time.Hour))
	require.NoError(t, err)
	reader, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: narrowRange})
	require.NoError(t, err)
...
func TestPlan_UnsupportedRawIntervalErrors(t *testing.T) {
	mgr := newTestManagerWithRaw(t, t.TempDir())
	odd, err := marketdata.NewInterval(marketdata.UnitHour, 7) // valid Interval, but not one of M1/H1/H4/D1
	require.NoError(t, err)
	span, err := marketdata.NewTimeRange(aWeekday(0), aWeekday(1))
	require.NoError(t, err)
