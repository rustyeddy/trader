...
import (
	...
	"github.com/rustyeddy/trader/internal/chart"
...
func runSlopeRetraceFullNotionalBacktest(ctx context.Context, mgr *marketruntime.Manager, simResolver instrument.Resolver, simID instrument.ID,
...
func TestSMATrendSlopeRetraceReEntryComparison(t *testing.T) {
	...
	for _, variant := range slopeRetraceVariants {
		...
		all := make([]runtimeorder.Trade, 0, len(resp.Trades)+len(resp.OpenTrades))
...
// exactly.
func slopeRetraceClassifyEpisode(t *testing.T, bars []marketdata.Bar, tr, next runtimeorder.Trade, fills map[id.FillID]runtimeorder.Fill) (*slopeRetraceEpisodeOutcome, error) {
	t.Helper()
