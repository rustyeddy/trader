...
// reconstruction.
func (r *wfMemoryRecorder) fillsByID() map[id.FillID]runtimeorder.Fill {
	m := make(map[id.FillID]runtimeorder.Fill, len(r.records))
...
func runWFBacktestWithJournal(ctx context.Context, mgr *marketruntime.Manager, simResolver instrument.Resolver, simID instrument.ID,
...
// directly.
func weightedFillPrice(t *testing.T, fillIDs []id.FillID, fills map[id.FillID]runtimeorder.Fill) (num.Price, num.Quantity, error) {
	t.Helper()
	if len(fillIDs) == 0 {
...
func foldTradeRows(t *testing.T, tsp string, fold int, testStart, testEnd time.Time, resp svcbacktest.RunResponse, fills map[id.FillID]runtimeorder.Fill) []tradeLogRow {
	t.Helper()
	var rows []tradeLogRow

	appendRow := func(tr runtimeorder.Trade, stillOpen bool) {
