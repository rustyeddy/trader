...
func intervalToken(i marketdata.Interval) (string, error) {
	switch i {
	case marketdata.M1:
		return "m1", nil
	case marketdata.H1:
...
func (s *canonicalCSVStore) publish(ctx context.Context, key partitionKey, m marketdata.Manifest, bs marketdata.BarSet) error {
...
func checkKeyMatchesManifest(key partitionKey, m marketdata.Manifest) error {
...
func (s *canonicalCSVStore) load(ctx context.Context, key partitionKey) (marketdata.Manifest, marketdata.BarSet, error) {
	if err := ctx.Err(); err != nil {
		return marketdata.Manifest{}, marketdata.BarSet{}, err
	}
	if err := key.validate(); err != nil {
		return marketdata.Manifest{}, marketdata.BarSet{}, fmt.Errorf("marketdata: store: load: %w", err)
	}
...
type manifestJSON struct {
	...
	IntervalUnit     marketdata.Unit             `json:"interval_unit"`
	IntervalCount    int                         `json:"interval_count"`
	SpanStart        string                      `json:"span_start"`
	SpanEnd          string                      `json:"span_end"`
	Basis            marketdata.PriceBasis       `json:"basis"`
	AdjustmentPolicy marketdata.AdjustmentPolicy `json:"adjustment_policy"`
...
func encodePartition(ctx context.Context, w *bufio.Writer, key partitionKey, m marketdata.Manifest, bs marketdata.BarSet) error {
...
func manifestToJSON(m marketdata.Manifest) manifestJSON {
	h := manifestJSON{
		Provider:         m.Provider,
...
// bars in file order.
func readPartitionFile(path string, key partitionKey) (marketdata.Manifest, []marketdata.Bar, error) {
	f, err := os.Open(path)
	if err != nil {
	...
	for scanner.Scan() {
		...
		if err != nil {
			return marketdata.Manifest{}, nil, fmt.Errorf("%w: %s:%d: %v", errStoreMalformed, path, line, err)
		}
		bars = append(bars, b)
	}
...
func parseBarRow(row string) (marketdata.Bar, error) {
	fields := strings.Split(row, ",")
	if len(fields) != 8 {
		return marketdata.Bar{}, fmt.Errorf("expected 8 fields, got %d", len(fields))
	}
	t, err := time.Parse(time.RFC3339Nano, fields[0])
	if err != nil {
		return marketdata.Bar{}, fmt.Errorf("time: %w", err)
	}
	...
	for i, name := range names {
		...
		if err != nil {
			return marketdata.Bar{}, fmt.Errorf("%s: %w", name, err)
		}
		prices[i] = p
	}
	ticks, err := strconv.ParseInt(fields[7], 10, 64)
	if err != nil {
		return marketdata.Bar{}, fmt.Errorf("ticks: %w", err)
	}
...
func decodeManifestJSON(line, path string, expectedInstrument instrument.ID) (marketdata.Manifest, error) {
	var h manifestJSON
	if err := json.Unmarshal([]byte(line), &h); err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: manifest header: %v", errStoreMalformed, path, err)
	}
	if h.Instrument != expectedInstrument.String() {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: manifest instrument %q disagrees with expected %q",
			errStoreMalformed, path, h.Instrument, expectedInstrument.String())
	}

	interval, err := marketdata.NewInterval(h.IntervalUnit, h.IntervalCount)
	if err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: interval: %v", errStoreMalformed, path, err)
	}
	spanStart, err := time.Parse(time.RFC3339Nano, h.SpanStart)
	if err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: span_start: %v", errStoreMalformed, path, err)
	}
	spanEnd, err := time.Parse(time.RFC3339Nano, h.SpanEnd)
	if err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: span_end: %v", errStoreMalformed, path, err)
	}
	span, err := marketdata.NewTimeRange(spanStart.UTC(), spanEnd.UTC())
	if err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: span: %v", errStoreMalformed, path, err)
	}
	builtAt, err := time.Parse(time.RFC3339Nano, h.BuiltAt)
	if err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: built_at: %v", errStoreMalformed, path, err)
	}
	firstBar, err := parseOptionalTime(h.FirstBar)
	if err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: first_bar: %v", errStoreMalformed, path, err)
	}
	lastBar, err := parseOptionalTime(h.LastBar)
	if err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: last_bar: %v", errStoreMalformed, path, err)
	}
	...
	if h.Parent != nil {
		if h.Parent.Instrument != expectedInstrument.String() {
			return marketdata.Manifest{}, fmt.Errorf("%w: %s: parent instrument %q disagrees with expected %q",
				errStoreMalformed, path, h.Parent.Instrument, expectedInstrument.String())
		}
		parentInterval, err := marketdata.NewInterval(h.Parent.IntervalUnit, h.Parent.IntervalCount)
		if err != nil {
			return marketdata.Manifest{}, fmt.Errorf("%w: %s: parent interval: %v", errStoreMalformed, path, err)
		}
		m.Parent = &marketdata.ParentRef{
			Instrument: expectedInstrument,
	...
	if err := m.Validate(); err != nil {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: %v", errStoreMalformed, path, err)
	}
	if got, want := m.Revision(), h.Revision; got != want {
		return marketdata.Manifest{}, fmt.Errorf("%w: %s: recomputed revision %s disagrees with stored revision %s",
