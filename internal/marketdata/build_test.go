...
// bars instead of 5).
func fullWeekD1Bars(t *testing.T, mgr *Manager, weekSpan marketdata.TimeRange) []marketdata.Bar {
	t.Helper()
	var bars []marketdata.Bar
	cursor := weekSpan.Start()
	for cursor.Before(weekSpan.End()) {
		daySpan, err := mgr.calendar.Bar(cursor, marketdata.D1)
		require.NoError(t, err)
		if mgr.calendar.Status(daySpan.Start()) == marketdata.StatusOpen {
...
func normalizeAction(year int, month time.Month, interval marketdata.Interval) Action {
...
func TestBuild_NormalizePublishesFromRaw(t *testing.T) {
	...
	plan := Plan{Actions: []Action{normalizeAction(2024, time.January, marketdata.H1)}}
	...
	assert.Equal(t, "none", m.ResamplerVersion)
	...
	span, err := marketdata.NewTimeRange(aWeekday(0), aWeekday(2))
	require.NoError(t, err)
	reader, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
	require.NoError(t, err)
	var got []marketdata.Bar
	for {
...
func TestBuild_NormalizeUnresolvedInstrumentErrors(t *testing.T) {
	mgr := newTestManagerWithRaw(t, t.TempDir())
	plan := Plan{Actions: []Action{{Kind: ActionNormalizeCanonical, Instrument: gbpusd(), Interval: marketdata.H1, Year: 2024, Month: time.January}}}
	_, err := mgr.Build(context.Background(), plan)
	assert.Error(t, err)
}
...
func TestBuild_DeriveUnresolvedInstrumentErrors(t *testing.T) {
	mgr := newTestManagerWithRaw(t, t.TempDir())
	plan := Plan{Actions: []Action{{Kind: ActionDeriveCanonical, Instrument: gbpusd(), Interval: marketdata.W1, Year: 2024, Month: time.January}}}
	_, err := mgr.Build(context.Background(), plan)
	assert.Error(t, err)
}
...
func TestBuild_NormalizeUnsupportedInterval(t *testing.T) {
	mgr := newTestManagerWithRaw(t, t.TempDir())
	odd, err := marketdata.NewInterval(marketdata.UnitHour, 7)
	require.NoError(t, err)
...
func TestBuild_DerivePublishesCompleteWeekOnly(t *testing.T) {
	mgr := newTestManagerWithRaw(t, t.TempDir())
	weekSpan, err := mgr.calendar.Bar(aWeekday(0), marketdata.W1)
	require.NoError(t, err)
	d1Bars := fullWeekD1Bars(t, mgr, weekSpan)
	d1Manifest := publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, weekSpan, d1Bars, validRawFingerprint, nil)

	plan := Plan{Actions: []Action{{Kind: ActionDeriveCanonical, Instrument: eurusd(), Interval: marketdata.W1, Year: 2024, Month: time.January}}}
	result, err := mgr.Build(context.Background(), plan)
	require.NoError(t, err)
	require.Len(t, result.Published, 1)
	...
	assert.Equal(t, marketdata.D1, w1.Parent.Interval)
	...
	reader, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: weekSpan})
	require.NoError(t, err)
	b, err := reader.Next(context.Background())
	require.NoError(t, err)
...
func TestBuild_DeriveSkipsWeekWithPartialD1(t *testing.T) {
	...
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, weekSpan, partial, validRawFingerprint, nil)
...
func TestBuild_DeriveWorksWithNoRawRoot(t *testing.T) {
	...
	require.NoError(t, err)
	d1Bars := fullWeekD1Bars(t, mgr, weekSpan)
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, weekSpan, d1Bars, validRawFingerprint, nil)
...
func TestBuild_PlanWithOnlySkippedActionsReportsThemWithoutRawRoot(t *testing.T) {
	...
	plan := Plan{Actions: []Action{
		{Kind: ActionDownloadRaw, Instrument: eurusd(), Interval: marketdata.H1, Year: 2024, Month: time.January},
	}}
	result, err := mgr.Build(context.Background(), plan)
	require.NoError(t, err)
...
// on).
func deriveMonthSpan(t *testing.T, year int, month time.Month) marketdata.TimeRange {
	t.Helper()
	start := time.Date(year, month, 1, 0, 0, 0, 0, time.UTC)
	span, err := marketdata.NewTimeRange(start, start.AddDate(0, 1, 0))
	require.NoError(t, err)
	return span
}
...
func lastWeekSpanOfMonth(t *testing.T, mgr *Manager, year int, month time.Month) marketdata.TimeRange {
	t.Helper()
...
func splitBarsAtMonthBoundary(bars []marketdata.Bar, boundary time.Time) (before, atOrAfter []marketdata.Bar) {
	for _, b := range bars {
...
func TestBuild_DeriveSkipsBoundaryWeekWhenNextMonthD1Missing(t *testing.T) {
	...
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, deriveMonthSpan(t, 2024, time.January), janBars, validRawFingerprint, nil)
...
func TestBuild_DeriveSkipsBoundaryWeekWhenNextMonthD1Invalid(t *testing.T) {
	...
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, deriveMonthSpan(t, 2024, time.January), janBars, validRawFingerprint, nil)
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.February, deriveMonthSpan(t, 2024, time.February), febBars, validRawFingerprint, nil)

	// Corrupt the published February D1 partition file directly, the
	// same technique TestCoverage_InvalidPartition uses.
	febKey := partitionKey{provider: "oanda", symbol: "EURUSD", instrument: eurusd(), interval: marketdata.D1, year: 2024, month: time.February}
...
func TestBuild_DeriveCombinesParentLineageAcrossMonthBoundary(t *testing.T) {
	...
	require.NotEmpty(t, febBars, "fixture assumption: the boundary week has at least one February day")
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, deriveMonthSpan(t, 2024, time.January), janBars, validRawFingerprint, nil)
	febManifest := publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.February, deriveMonthSpan(t, 2024, time.February), febBars, validRawFingerprint, nil)
	...
	cov, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: deriveMonthSpan(t, 2024, time.January)})
	require.NoError(t, err)
	require.Len(t, cov.Partitions, 1)
	...
	extraDay, err := mgr.calendar.Bar(boundaryWeek.End(), marketdata.D1)
	require.NoError(t, err)
	febBars2 := append(append([]marketdata.Bar{}, febBars...), barAt(t, extraDay.Start()))
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.February, deriveMonthSpan(t, 2024, time.February), febBars2, validRawFingerprint, nil)

	cov2, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: deriveMonthSpan(t, 2024, time.January)})
	require.NoError(t, err)
...
func TestBuild_SkipsNonCanonicalActions(t *testing.T) {
	...
	plan := Plan{Actions: []Action{
		{Kind: ActionDownloadRaw, Instrument: eurusd(), Interval: marketdata.H1, Year: 2024, Month: time.January},
		{Kind: ActionRepairRaw, Instrument: eurusd(), Interval: marketdata.H1, Year: 2024, Month: time.January},
	}}
	result, err := mgr.Build(context.Background(), plan)
	require.NoError(t, err)
	assert.Empty(t, result.Published)
...
func TestBuild_RepublishInvalidatesCache(t *testing.T) {
	...
	require.NoError(t, err)
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, []marketdata.Bar{barAt(t, aWeekday(0))}, validRawFingerprint, nil)
	...
	writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2024, time.January, rawRow(aWeekday(0), true))
	plan := Plan{Actions: []Action{normalizeAction(2024, time.January, marketdata.H1)}}
	_, err = mgr.Build(context.Background(), plan)
	require.NoError(t, err)

	reader2, err := mgr.Bars(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
	require.NoError(t, err)
...
func TestBuild_CancelledContext(t *testing.T) {
	...
	cancel()
	_, err := mgr.Build(ctx, Plan{Actions: []Action{normalizeAction(2024, time.January, marketdata.H1)}})
...
func TestBuild_CancelledMidLoop(t *testing.T) {
	rawRoot := t.TempDir()
	writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2024, time.January, rawRow(aWeekday(0), true))
	writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2024, time.February, rawRow(aWeekday(0).AddDate(0, 1, 0), true))
	mgr := newTestManagerWithRaw(t, rawRoot)

	ctx := &nthCancelContext{Context: context.Background(), remaining: 1}
	plan := Plan{Actions: []Action{
		normalizeAction(2024, time.January, marketdata.H1),
		normalizeAction(2024, time.February, marketdata.H1),
	}}
