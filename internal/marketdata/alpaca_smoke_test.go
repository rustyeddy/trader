...
func newSmokeAlpacaManager(t *testing.T) *Manager {
	...
	mgr, err := New(Config{
		...
		Resolver:         r,
		ProviderName:     "alpaca",
		Calendar:         marketdata.NewUSEquityCalendar(marketdata.StandardUSEquityHolidays(testUSEquityCalendarYears()...)),
...
func smokeRecentRange(t *testing.T, daysBack, marginDays int) marketdata.TimeRange {
	t.Helper()
...
func readAllBars(t *testing.T, ctx context.Context, reader *BarReader) []marketdata.Bar {
	t.Helper()
...
func assertBarsWellFormed(t *testing.T, bars []marketdata.Bar) {
	t.Helper()
...
func TestSmokeAlpacaEndToEndSPY(t *testing.T) {
	...
	query := BarQuery{Instrument: spy, Interval: marketdata.D1, Range: span}
	...
	extended, err := marketdata.NewTimeRange(span.Start(), span.End().AddDate(0, 0, 3))
	require.NoError(t, err)
	extQuery := BarQuery{Instrument: spy, Interval: marketdata.D1, Range: extended}
	...
	extendedByTime := make(map[time.Time]marketdata.Bar, len(extendedBars))
...
func TestSmokeAlpacaEndToEndAAPL(t *testing.T) {
	...
	query := BarQuery{Instrument: aapl, Interval: marketdata.D1, Range: span}

	buildResult := syncThenBuild(t, ctx, mgr, query)
	require.NotEmpty(t, buildResult.Published)
	for _, pr := range buildResult.Published {
		assert.Equal(t, marketdata.AdjustmentSplitAdjusted, pr.Manifest.AdjustmentPolicy, "AAPL bars must be recorded as split-adjusted")
...
func TestSmokeAlpacaAAPLSplitAdjustment(t *testing.T) {
	...
	byDate := make(map[string]marketdata.Bar, len(bars))
	for _, b := range bars {
...
func TestSmokeAlpacaVsStooqComparison(t *testing.T) {
	...
	stooqByDate := make(map[string]marketdata.Bar, len(stooqBars))
