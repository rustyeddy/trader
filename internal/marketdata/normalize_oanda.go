...
func canonicalInterval(raw oanda.RawInterval) (marketdata.Interval, error) {
	switch raw {
	case oanda.RawM1:
	...
	default:
		return marketdata.Interval{}, fmt.Errorf("marketdata: normalize: %w: %q", errUnsupportedRawInterval, raw)
	}
}
...
func normalizeOANDASequence(rawInterval oanda.RawInterval, cal marketdata.Calendar, records []oanda.Record) ([]normalizedRecord, error) {
	if cal == nil {
		return nil, fmt.Errorf("marketdata: normalize sequence: %w", marketdata.ErrNilCalendar)
	}
