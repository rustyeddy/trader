...
func rawTFToken(i marketdata.Interval) string {
	switch i {
	case marketdata.M1:
		return "m1"
	case marketdata.H1:
...
func writeRawPartition(t *testing.T, root, symbol string, interval marketdata.Interval, year int, month time.Month, rows ...string) string {
	t.Helper()
...
func publishCanonicalMonth(t *testing.T, mgr *Manager, interval marketdata.Interval, year int, month time.Month, span marketdata.TimeRange, bars []marketdata.Bar, fingerprint string, parent *marketdata.ParentRef) marketdata.Manifest {
	t.Helper()
	...
	bs := marketdata.BarSet{Instrument: eurusd(), Interval: interval, Span: span, Basis: marketdata.BasisBid, Bars: bars}
...
func TestCoverage_MissingPartition(t *testing.T) {
	...
	span, err := marketdata.NewTimeRange(aWeekday(0), aWeekday(4))
	require.NoError(t, err)

	cov, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
	require.NoError(t, err)
...
func TestCoverage_InvalidPartition(t *testing.T) {
	...
	bars := []marketdata.Bar{barAt(t, aWeekday(0)), barAt(t, aWeekday(1))}
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, validRawFingerprint, nil)
...
func TestCoverage_CurrentPartitionRawFingerprintMatches(t *testing.T) {
	...
	rawPath := writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2024, time.January,
		rawRow(aWeekday(0), true), rawRow(aWeekday(1), true))
	fp := rawFingerprint(t, rawPath)

	bars := []marketdata.Bar{barAt(t, aWeekday(0)), barAt(t, aWeekday(1))}
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, fp, nil)
...
func TestCoverage_RawIncompleteCount(t *testing.T) {
	...
	bars := []marketdata.Bar{barAt(t, aWeekday(0)), barAt(t, aWeekday(1)), barAt(t, aWeekday(2))}
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, fp, nil)
...
func TestCoverage_W1StaleWhenParentRevisionChanges(t *testing.T) {
	...
	d1Bars := []marketdata.Bar{barAt(t, aWeekday(0))}
	d1Manifest := publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, span, d1Bars, validRawFingerprint, nil)

	w1Bars := []marketdata.Bar{barAt(t, aWeekday(0))}
	parent := &marketdata.ParentRef{Instrument: eurusd(), Interval: marketdata.D1, Revision: d1Manifest.Revision()}
	publishCanonicalMonth(t, mgr, marketdata.W1, 2024, time.January, span, w1Bars, validRawFingerprint, parent)

	cov, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: span})
	require.NoError(t, err)
	require.Len(t, cov.Partitions, 1)
	...
	d1Key := partitionKey{provider: "oanda", symbol: "EURUSD", instrument: eurusd(), interval: marketdata.D1, year: 2024, month: time.January}
	mgr.cache.invalidate(d1Key)
	d1Bars2 := []marketdata.Bar{barAt(t, aWeekday(0)), barAt(t, aWeekday(1))}
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, span, d1Bars2, validRawFingerprint, nil)

	cov2, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: span})
	require.NoError(t, err)
...
func TestCoverage_GapsExcludeClosedWeekend(t *testing.T) {
	...
	rawPath := writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2024, time.January, rawRow(start, true))
	fp := rawFingerprint(t, rawPath)
	...
	bars := []marketdata.Bar{barAt(t, start)}
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, fp, nil)

	cov, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
	require.NoError(t, err)
	for _, g := range cov.Gaps {
		assert.NotEqual(t, marketdata.IntervalStateClosed, g.State, "a closed weekend must never appear as a Gap")
	}
}
...
func TestCoverage_H4SurvivesSpringForwardWeekend(t *testing.T) {
	...
	rawPath := writeRawPartition(t, rawRoot, "EURUSD", marketdata.H4, 2026, time.March, rawRow(start, true))
	fp := rawFingerprint(t, rawPath)
	bars := []marketdata.Bar{barAt(t, start)}
	publishCanonicalMonth(t, mgr, marketdata.H4, 2026, time.March, span, bars, fp, nil)

	cov, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H4, Range: span})
...
func TestCoverage_GapsMergeContiguousMissingBoundaries(t *testing.T) {
	...
	rawPath := writeRawPartition(t, rawRoot, "EURUSD", marketdata.H1, 2024, time.January, rawRow(start, true), rawRow(aWeekday(4), true))
	fp := rawFingerprint(t, rawPath)

	// Only the first and last hour are present; hours 1,2,3 are missing
	// and must merge into exactly one Gap.
	bars := []marketdata.Bar{barAt(t, aWeekday(0)), barAt(t, aWeekday(4))}
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, fp, nil)
...
func TestCoverage_RawRootRequiredForRawBuiltInterval(t *testing.T) {
	...
	_, err = mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
...
func TestCoverage_InvalidQuery(t *testing.T) {
	...
	_, err = mgr.Coverage(context.Background(), BarQuery{Instrument: instrument.ID{}, Interval: marketdata.H1, Range: span})
...
func TestCoverage_CancelledContext(t *testing.T) {
	...
	_, err = mgr.Coverage(ctx, BarQuery{Instrument: eurusd(), Interval: marketdata.H1, Range: span})
...
func TestCoverage_UnresolvedInstrument(t *testing.T) {
	...
	_, err = mgr.Coverage(context.Background(), BarQuery{Instrument: gbpusd(), Interval: marketdata.H1, Range: span})
	assert.ErrorIs(t, err, instrument.ErrUnknownSymbol)
}

func TestIntervalToRawInterval(t *testing.T) {
	cases := []struct {
		in marketdata.Interval
		ok bool
	}{
		{marketdata.M1, true}, {marketdata.H1, true}, {marketdata.H4, true}, {marketdata.D1, true}, {marketdata.W1, false},
	}
	for _, c := range cases {
...
func TestCoverage_CurrentNotStaleWhenRawUnavailable(t *testing.T) {
	...
	mgr := newTestManagerWithRaw(t, rawRoot)
	span, err := marketdata.NewTimeRange(aWeekday(0), aWeekday(1))
	require.NoError(t, err)

	bars := []marketdata.Bar{barAt(t, aWeekday(0))}
	publishCanonicalMonth(t, mgr, marketdata.H1, 2024, time.January, span, bars, validRawFingerprint, nil)
...
func TestCoverage_W1InProgressWeekIsAGap(t *testing.T) {
	...
	now := mgr.clock.Now()
	weekSpan, err := mgr.calendar.Bar(now, marketdata.W1)
	require.NoError(t, err)
	...
	parent := &marketdata.ParentRef{Instrument: eurusd(), Interval: marketdata.D1, Revision: "dummy-parent-revision"}
	utcStart := weekSpan.Start().UTC()
	publishCanonicalMonth(t, mgr, marketdata.W1, utcStart.Year(), utcStart.Month(), weekSpan, nil, validRawFingerprint, parent)

	cov, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.W1, Range: weekSpan})
	require.NoError(t, err)
	require.Len(t, cov.Partitions, 1)
	require.Equal(t, PartitionCoverageCurrent, cov.Partitions[0].Status)
	require.Len(t, cov.Gaps, 1)
	assert.Equal(t, marketdata.IntervalStateInProgress, cov.Gaps[0].State)
}
...
func TestCoverage_GapNeverStartsBeforeQueryRange(t *testing.T) {
	mgr := newTestManagerWithRaw(t, t.TempDir())
	day1, err := mgr.calendar.Bar(aWeekday(0), marketdata.D1)
	require.NoError(t, err)
	day2, err := mgr.calendar.Bar(day1.End(), marketdata.D1)
	require.NoError(t, err)
	...
	span, err := marketdata.NewTimeRange(day1.Start(), day2.End())
	require.NoError(t, err)
	publishCanonicalMonth(t, mgr, marketdata.D1, day1.Start().UTC().Year(), day1.Start().UTC().Month(), span, nil, validRawFingerprint, nil)

	queryRange, err := marketdata.NewTimeRange(queryStart, day2.End())
	require.NoError(t, err)
	cov, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.D1, Range: queryRange})
	require.NoError(t, err)
...
func TestCoverage_StraddlingMonthBoundaryNotDoubleCounted(t *testing.T) {
	...
	straddle, err := mgr.calendar.Bar(probe, marketdata.D1)
	require.NoError(t, err)
	...
	janSpan, err := marketdata.NewTimeRange(time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC), straddle.End())
	require.NoError(t, err)
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.January, janSpan, nil, validRawFingerprint, nil)

	febSpan, err := marketdata.NewTimeRange(straddle.End(), time.Date(2024, 2, 3, 0, 0, 0, 0, time.UTC))
	require.NoError(t, err)
	publishCanonicalMonth(t, mgr, marketdata.D1, 2024, time.February, febSpan, nil, validRawFingerprint, nil)

	queryRange, err := marketdata.NewTimeRange(time.Date(2024, 1, 30, 0, 0, 0, 0, time.UTC), time.Date(2024, 2, 2, 0, 0, 0, 0, time.UTC))
	require.NoError(t, err)
	cov, err := mgr.Coverage(context.Background(), BarQuery{Instrument: eurusd(), Interval: marketdata.D1, Range: queryRange})
	require.NoError(t, err)
