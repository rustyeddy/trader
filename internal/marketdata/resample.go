...
func (m *Manager) deriveAndPublish(ctx context.Context, action Action) (PublishResult, error) {
	...
	monthSpan, err := marketdata.NewTimeRange(monthStart, monthEnd)
	if err != nil {
	...
	d1Key := partitionKey{
		...
		interval: marketdata.D1, year: action.Year, month: action.Month,
	}
	...
	coverageRange, err := marketdata.NewTimeRange(monthStart, coverageEnd)
	if err != nil {
		return PublishResult{}, fmt.Errorf("marketdata: build: %w", err)
	}
	d1Query := BarQuery{Instrument: action.Instrument, Interval: marketdata.D1, Range: coverageRange}
	...
	for _, weekSpan := range weekSpans {
		...
		weekBars, err := m.readAllBars(ctx, BarQuery{Instrument: action.Instrument, Interval: marketdata.D1, Range: weekSpan})
		if err != nil {
	...
	bs := marketdata.BarSet{Instrument: action.Instrument, Interval: marketdata.W1, Span: monthSpan, Basis: marketdata.BasisBid, Bars: bars}
	...
	contributing := []marketdata.Manifest{parentManifest}
	if spansNext {
		nextKey := partitionKey{
			provider: m.providerName, symbol: symbol, instrument: action.Instrument,
			interval: marketdata.D1, year: monthEnd.Year(), month: monthEnd.Month(),
		}
	...
	if err := m.publishCanonicalMonth(ctx, symbol, marketdata.W1, action.Year, action.Month, manifest, bs); err != nil {
...
func weekSpansForMonth(cal marketdata.Calendar, monthStart, monthEnd time.Time) ([]marketdata.TimeRange, time.Time, error) {
	cursor, err := firstBoundaryAtOrAfter(cal, monthStart, marketdata.W1)
	if err != nil {
		return nil, time.Time{}, err
	}

	var spans []marketdata.TimeRange
	coverageEnd := monthEnd
	for cursor.Before(monthEnd) {
		weekSpan, err := cal.Bar(cursor, marketdata.W1)
		if err != nil {
			return nil, time.Time{}, err
		}
...
// rule separately.
func w1SpansNextMonth(cal marketdata.Calendar, lastBar time.Time, monthEnd time.Time) (bool, error) {
	if lastBar.IsZero() {
		return false, nil
	}
	lastWeek, err := cal.Bar(lastBar, marketdata.W1)
	if err != nil {
		return false, err
	}
...
func combineParentLineage(contributing []marketdata.Manifest) (revision, rawFingerprint string) {
...
func weekIsD1Ready(cov Coverage, weekSpan marketdata.TimeRange) bool {
...
func aggregateBars(bars []marketdata.Bar) (marketdata.Bar, error) {
	if len(bars) == 0 {
		return marketdata.Bar{}, fmt.Errorf("marketdata: aggregate: no input bars")
	}
	...
	for _, b := range bars {
		...
		if err != nil {
			return marketdata.Bar{}, fmt.Errorf("parse ticks as rate: %w", err)
		}
		contribution, err := b.AvgSpread.MulRate(weight)
		if err != nil {
			return marketdata.Bar{}, fmt.Errorf("weight spread: %w", err)
		}
		if !haveWeighted {
		...
		if err != nil {
			return marketdata.Bar{}, fmt.Errorf("sum weighted spread: %w", err)
		}
	}
	...
	if totalTicks > 0 {
		...
		if err != nil {
			return marketdata.Bar{}, fmt.Errorf("parse total ticks as rate: %w", err)
		}
		inv, err := num.MustParseRate("1").DivRate(totalRate)
		if err != nil {
			return marketdata.Bar{}, fmt.Errorf("compute weight inverse: %w", err)
		}
		avg, err := weightedSum.MulRate(inv)
		if err != nil {
			return marketdata.Bar{}, fmt.Errorf("average spread: %w", err)
		}
		agg.AvgSpread = avg
	}
