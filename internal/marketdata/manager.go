...
type barStore interface {
	...
	publish(ctx context.Context, key partitionKey, m marketdata.Manifest, bs marketdata.BarSet) error
	// load reads the current published (Manifest, BarSet) pair for key,
	// verifying they still Match before returning either.
	load(ctx context.Context, key partitionKey) (marketdata.Manifest, marketdata.
