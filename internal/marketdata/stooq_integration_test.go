...
func testUSEquityCalendar() *marketdata.USEquityCalendar {
	return marketdata.NewUSEquityCalendar(marketdata.StandardUSEquityHolidays(testUSEquityCalendarYears()...))
}
...
func TestStooqEndToEnd_AAPLRecordsSplitAdjustedPolicy(t *testing.T) {
	...
	require.NoError(t, err)
	query := BarQuery{Instrument: aaplID(t), Interval: marketdata.D1, Range: span}
