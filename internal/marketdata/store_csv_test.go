...
func TestIntervalToken(t *testing.T) {
	cases := map[marketdata.Interval]string{marketdata.M1: "m1", marketdata.H1: "h1", marketdata.H4: "h4", marketdata.D1: "d1", marketdata.W1: "w1"}
...
func TestIntervalToken_Unsupported(t *testing.T) {
	weird, err := marketdata.NewInterval(marketdata.UnitMinute, 7)
	require.NoError(t, err)
...
func TestCanonicalCSVStore_ExactDecimalPrecision(t *testing.T) {
	...
	key := partitionKey{provider: "oanda", symbol: "USDJPY", instrument: instrumentUSDJPY(), interval: marketdata.H1, year: 2020, month: time.May}
	...
	bs := marketdata.BarSet{Instrument: key.instrument, Interval: marketdata.H1, Span: span, Basis: marketdata.BasisBid, Bars: []marketdata.Bar{bar}}
	m := validManifest(t)
...
func TestPartitionKey_ValidateRejectsTraversal(t *testing.T) {
	cases := []partitionKey{
		{provider: "../escape", symbol: "EURUSD", instrument: eurusd(), interval: marketdata.H1, year: 2020, month: time.May},
		{provider: "oanda", symbol: "../../escape", instrument: eurusd(), interval: marketdata.H1, year: 2020, month: time.May},
		{provider: "oanda/sub", symbol: "EURUSD", instrument: eurusd(), interval: marketdata.H1, year: 2020, month: time.May},
	}
...
func TestCanonicalCSVStore_PublishRejectsKeyIntervalMismatch(t *testing.T) {
	store := newCanonicalCSVStore(t.TempDir())
	key := validPartitionKey(t)
	key.interval = marketdata. // validManifest/validBarSet use H1
					D1
	err := store.publish(context.Background(), key, validManifest(t), validBarSet(t))
	assert.ErrorIs(t, err, errStorePartitionKeyMismatch)
}
...
func TestPartitionKey_ValidateRejectsZeroInstrumentAndInvalidInterval(t *testing.T) {
	...
	key2.interval = marketdata.Interval{}
...
func TestDecodeManifestJSON_MalformedFields(t *testing.T) {
	...
	mutate := map[string]func(*manifestJSON){
		"interval_unit": func(h *manifestJSON) { h.IntervalUnit = marketdata.Unit(200) },
		...
		"parent interval": func(h *manifestJSON) {
			h.Parent = &parentJSON{Instrument: m.Instrument.String(), IntervalUnit: marketdata.Unit(200), IntervalCount: 1, Revision: "r"}
		},
	}
