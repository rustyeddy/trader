...
func (r *BarReader) Next(ctx context.Context) (marketdata.Bar, error) {
	if r == nil || r.closed {
		return marketdata.Bar{}, errBarReaderClosed
	}
	if err := ctx.Err(); err != nil {
		return marketdata.Bar{}, err
	}
	if r.pos >= len(r.bars) {
		return marketdata.Bar{}, io.EOF
	}
	b := r.bars[r.pos]
	r.pos++
	return b, nil
...
func (r *BarReader) Manifests() []marketdata.Manifest {
	if r == nil {
		return nil
	}
	out := make([]marketdata.Manifest, len(r.manifests))
...
func (m *Manager) Bars(ctx context.Context, query BarQuery) (*BarReader, error) {
	...
	seen := make(map[time.Time]marketdata.Bar)
	manifests := make([]marketdata.Manifest, 0, len(keys))
...
func (m *Manager) readAllBars(ctx context.Context, query BarQuery) ([]marketdata.Bar, error) {
...
// ErrInconsistentData.
func barsEqual(a, b marketdata.Bar) bool {
...
func monthPartitionKeys(provider, symbol string, id instrument.ID, interval marketdata.Interval, r marketdata.TimeRange) []partitionKey {
	start := r.Start().UTC()
	end := r.End().UTC()
...
func coverageGap(want marketdata.TimeRange, spans []marketdata.TimeRange) (marketdata.TimeRange, bool) {
	...
	for _, iv := range clipped {
		if iv.start.After(cursor) {
			gap, err := marketdata.NewTimeRange(cursor, iv.start)
			if err != nil {
	...
	if cursor.Before(want.End()) {
		gap, err := marketdata.NewTimeRange(cursor, want.End())
		if err != nil {
			panic(fmt.Sprintf("marketdata: coverageGap: %v", err))
		}
		return gap, false
