...
// IntervalStateClosed.
type Gap struct {
	State marketdata.IntervalState
...
func intervalToRawInterval(i marketdata.Interval) (string, bool) {
	switch i {
	case marketdata.M1:
		return "m1", true
	case marketdata.H1:
...
func (m *Manager) isStale(ctx context.Context, key partitionKey, man marketdata.Manifest, symbol string, rawByKey map[rawPartitionKey]rawPartitionInfo, rawInterval string, rawApplicable bool) (bool, error) {
	if key.interval == marketdata.W1 {
		parentKey := key
		parentKey.interval = marketdata.D1
		...
		contributing := []marketdata.Manifest{parentMan}
		if spansNext {
...
func (a *gapAccumulator) add(state marketdata.IntervalState, span marketdata.TimeRange) error {
	if state == marketdata.IntervalStatePresent || state == marketdata.IntervalStateClosed {
		a.flush()
		return nil
	}
	if a.open != nil && a.open.State == state && a.open.Span.End().Equal(span.Start()) {
		merged, err := marketdata.NewTimeRange(a.open.Span.Start(), span.End())
		if err != nil {
			return err
		}
		a.open.Span = merged
...
func firstBoundaryAtOrAfter(cal marketdata.Calendar, at time.Time, interval marketdata.Interval) (time.Time, error) {
...
func (m *Manager) gapsForPartition(key partitionKey, bs marketdata.BarSet, queryRange marketdata.TimeRange) ([]Gap, error) {
	...
	for cursor.Before(clipEnd) {
		...
		state, err := marketdata.ClassifyInterval(m.calendar, span, now, present[span.Start().UTC().UnixNano()], true)
		if err != nil {
			return nil, fmt.Errorf("marketdata: coverage: %w", err)
		}
...
func (m *Manager) gapsForWeeklyPartition(present map[int64]bool, clipStart, clipEnd time.Time) ([]Gap, error) {
	cursor, err := firstBoundaryAtOrAfter(m.calendar, clipStart, marketdata.W1)
	if err != nil {
		return nil, fmt.Errorf("marketdata: coverage: %w", err)
	}
	now := m.clock.Now()
	...
	acc := &gapAccumulator{}
	for cursor.Before(clipEnd) {
		span, err := m.calendar.Bar(cursor, marketdata.W1)
		if err != nil {
			return nil, fmt.Errorf("marketdata: coverage: %w", err)
		}
		var state marketdata.IntervalState
		switch {
		case !span.Elapsed(now):
			state = marketdata.IntervalStateInProgress
		case present[span.Start().UTC().UnixNano()]:
			state = marketdata.IntervalStatePresent
		default:
			state = marketdata.IntervalStateMissing
		}
		if err := acc.add(state, span); err != nil {
			return nil, fmt.Errorf("marketdata: coverage: %w", err)
		}
