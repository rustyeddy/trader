...
func (m *Manager) needsExtend(p rawPartitionInfo, queryRange marketdata.TimeRange, interval marketdata.Interval) (bool, error) {
	...
	for next.Before(upper) {
		...
		if m.calendar.Status(nspan.Start()) == marketdata.StatusOpen {
			return true, nil
		}
		next = nspan.End()
	}
	...
	for _, pc := range cov.Partitions {
		...
		if pc.Status == PartitionCoverageCurrent {
			...
			actions = append(actions, Action{
				Kind: ActionDeriveCanonical, Instrument: query.Instrument, Interval: marketdata.W1,
		...
		d1Range, err := marketdata.NewTimeRange(clipStart, clipEnd)
		if err != nil {
			return nil, fmt.Errorf("marketdata: plan: %w", err)
		}

		d1Query := BarQuery{Instrument: query.Instrument, Interval: marketdata.D1, Range: d1Range}
...
func (m *Manager) w1CurrentPartitionNeedsDerive(ctx context.Context, query BarQuery, cov Coverage, monthStart, monthEnd time.Time, symbol string) (bool, error) {
	...
	d1Range, err := marketdata.NewTimeRange(monthStart, coverageEnd)
	if err != nil {
		return false, fmt.Errorf("marketdata: plan: %w", err)
	}
	d1Query := BarQuery{Instrument: query.Instrument, Interval: marketdata.D1, Range: d1Range}
...
func overlappingGapSpans(gaps []Gap, start, end time.Time) []marketdata.TimeRange {
