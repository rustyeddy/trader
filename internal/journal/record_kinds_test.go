...
func TestNewRecordValidEveryKind(t *testing.T) {
	...
	ord, err := runtimeorder.NewOrder(runtimeorder.Order{Request: request, AcceptedQuantity: &accepted, Status: runtimeorder.StatusWorking})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	trade, err := runtimeorder.NewTrade(runtimeorder.Trade{
