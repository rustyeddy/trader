...
func mustSnapshotWithPositions(t *testing.T, accountID id.AccountID, broker, currency, equity string, positions ...runtimeorder.Position) account.Snapshot {
	t.Helper()
	cur := num.MustParseCurrency(currency)
	snap, err := account.NewSnapshot(account.SnapshotParams{
		AccountID:       accountID,
...
func mustProposal(t *testing.T, accountID id.AccountID, listing instrument.Listing) runtimeorder.Proposal {
	t.Helper()
...
func mustProposalWith(t *testing.T, accountID id.AccountID, listing instrument.Listing, side order.Side, quantity string, reduceOnly bool) runtimeorder.Proposal {
	t.Helper()
	p, err := runtimeorder.NewProposal(runtimeorder.Proposal{
		Listing:     listing,
		AccountID:   accountID,
