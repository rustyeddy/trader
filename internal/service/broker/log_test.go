...
func TestCancel_LogsOrderIDEvenWhenAccountOpenFails(t *testing.T) {
	...
	_, err = svc.Cancel(context.Background(), CancelRequest{
		...
		Cancel:         runtimeorder.CancelRequest{OrderID: orderID},
	})
	require.Error(t, err)
...
func TestService_LogsOnlyKnownAttributeKeys(t *testing.T) {
	...
	_, _ = svc.Replace(ctx, ReplaceRequest{AccountRequest: AccountRequest{AccountID: accountID}, Replace: runtimeorder.ReplaceRequest{OrderID: limitReq.OrderID, NewQuantity: &newQty}})
	_, _ = svc.Cancel(ctx, CancelRequest{AccountRequest: AccountRequest{AccountID: accountID}, Cancel: runtimeorder.CancelRequest{OrderID: limitReq.OrderID}})
