...
func mustMarketRequest(t *testing.T, gen *id.Generator, accountID id.AccountID) runtimeorder.Request {
	t.Helper()
...
func mustLimitRequest(t *testing.T, gen *id.Generator, accountID id.AccountID) runtimeorder.Request {
	t.Helper()
	eventID, err := id.GenerateEventID(gen)
	require.NoError(t, err)
...
func TestServiceSubmit(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusFilled, resp.Order.Status)
}
...
func TestServiceCancel(t *testing.T) {
	...
	require.NoError(t, err)
	cancelReq, err := runtimeorder.NewCancelRequest(runtimeorder.CancelRequest{OrderID: submitReq.OrderID, Metadata: id.Metadata{EventID: cancelEventID}})
	require.NoError(t, err)

	resp, err := svc.Cancel(ctx, CancelRequest{AccountRequest: AccountRequest{AccountID: accountID}, Cancel: cancelReq})
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusCanceled, resp.Result.Status)
...
func TestServiceCancelUnknownOrder(t *testing.T) {
	...
	cancelEventID, err := id.GenerateEventID(gen)
	require.NoError(t, err)
	cancelReq, err := runtimeorder.NewCancelRequest(runtimeorder.CancelRequest{OrderID: unknownOrder, Metadata: id.Metadata{EventID: cancelEventID}})
	require.NoError(t, err)
...
func TestServiceReplace(t *testing.T) {
	...
	replaceReq, err := runtimeorder.NewReplaceRequest(runtimeorder.ReplaceRequest{OrderID: submitReq.OrderID, NewQuantity: &qty, Metadata: id.Metadata{EventID: replaceEventID}})
	require.NoError(t, err)

	resp, err := svc.Replace(ctx, ReplaceRequest{AccountRequest: AccountRequest{AccountID: accountID}, Replace: replaceReq})
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusWorking, resp.Result.Status)
	assert.Nil(t, resp.Result.Rejection)
...
func TestServiceReplaceUnknownOrder(t *testing.T) {
	...
	require.NoError(t, err)
	qty := num.MustParseQuantity("500")
	replaceReq, err := runtimeorder.NewReplaceRequest(runtimeorder.ReplaceRequest{OrderID: unknownOrder, NewQuantity: &qty, Metadata: id.Metadata{EventID: replaceEventID}})
	require.NoError(t, err)
...
func TestServiceReplaceRejectsZeroAccountID(t *testing.T) {
	...
	require.NoError(t, err)
	qty := num.MustParseQuantity("500")
	replaceReq, err := runtimeorder.NewReplaceRequest(runtimeorder.ReplaceRequest{OrderID: orderID, NewQuantity: &qty, Metadata: id.Metadata{EventID: eventID}})
	require.NoError(t, err)
...
func TestServiceCancelPropagatesOpenAccountError(t *testing.T) {
	...
	eventID, err := id.GenerateEventID(gen)
	require.NoError(t, err)
	cancelReq, err := runtimeorder.NewCancelRequest(runtimeorder.CancelRequest{OrderID: orderID, Metadata: id.Metadata{EventID: eventID}})
	require.NoError(t, err)
	require.NoError(t, b.Close())
	svc, err := New(b, nil)
	require.NoError(t, err)
