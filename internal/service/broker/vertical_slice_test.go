...
func TestVerticalSlice_ServiceComposesFullOrderLifecycle(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, runtimeorder.StatusFilled, submitResp.Order.Status)
	...
	require.NoError(t, err)
	require.Equal(t, runtimeorder.StatusWorking, pendingResp.Order.Status)

	eventID, err := id.GenerateEventID(gen)
	require.NoError(t, err)
	cancelReq, err := runtimeorder.NewCancelRequest(runtimeorder.CancelRequest{OrderID: pendingReq.OrderID, Metadata: id.Metadata{EventID: eventID}})
	require.NoError(t, err)
	cancelResp, err := svc.Cancel(ctx, CancelRequest{AccountRequest: AccountRequest{AccountID: accountID}, Cancel: cancelReq})
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusCanceled, cancelResp.Result.Status)
