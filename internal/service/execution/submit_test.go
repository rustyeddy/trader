...
func TestSubmit_EndToEndAgainstSimulator(t *testing.T) {
	...
	assert.NotEqual(t, runtimeorder.Order{}, resp.Order)
...
func TestEvaluate_EndToEndNeverMutatesBroker(t *testing.T) {
	...
	require.NoError(t, err)
	assert.True(t, resp.Decision.Allowed)
	assert.NotEqual(t, runtimeorder.Request{}, resp.Request)
	assert.False(t, resp.Request.OrderID.IsZero())
	assert.Equal(t, runtimeorder.Order{}, resp.Order, "Evaluate must never populate Order")
...
func TestEvaluate_RejectionLeavesRequestZero(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.Request{}, resp.Request)
	assert.Equal(t, runtimeorder.Order{}, resp.Order)
}
...
func TestSubmit_RiskRejectionReturnsStructuredResultButNoOrder(t *testing.T) {
	...
	require.ErrorIs(t, err, pipeline.ErrRejected)
	assert.False(t, resp.Decision.Allowed)
	assert.NotEqual(t, runtimeorder.Proposal{}, resp.Proposal)
	assert.Equal(t, runtimeorder.Order{}, resp.Order)
...
func (a failingAccount) Submit(ctx context.Context, req runtimeorder.Request) (runtimeorder.Order, error) {
	return runtimeorder.Order{}, errors.New("not implemented")
}
