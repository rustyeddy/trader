...
func runExecutionVerticalSliceScenario(t *testing.T) verticalSliceResult {
	...
	buildIntent := func(instID instrument.ID) runtimeorder.Intent {
...
func TestExecutionVerticalSlice_FullScenarioProducesExpectedState(t *testing.T) {
	...
	t.Run("account A: evaluate never mutates the broker", func(t *testing.T) {
		require.NotEqual(t, runtimeorder.Request{}, result.FirstEvaluate.Request)
		require.Equal(t, runtimeorder.Order{}, result.FirstEvaluate.Order, "Evaluate must never populate Order")
	})
	...
	t.Run("account A: approved submission is sized, filled, and matches hand-computed accounting", func(t *testing.T) {
		...
		require.Equal(t, runtimeorder.StatusFilled, result.Approved.Order.Status)
	...
	t.Run("account A: oversized second intent is rejected by real risk policy with no mutation", func(t *testing.T) {
		...
		assert.Equal(t, runtimeorder.Request{}, result.Rejected.Request, "Request must never be populated on rejection")
		assert.Equal(t, runtimeorder.Order{}, result.Rejected.Order, "Order must never be populated on rejection")
