...
func (noopStrategy) OnBar(ctx context.Context, event strategy.BarEvent, view strategy.View) ([]runtimeorder.Intent, error) {
	return nil, nil
}
...
func newFixtureManager(t *testing.T) *marketruntime.Manager {
	t.Helper()
	...
	plan, err := mgr.Plan(ctx, marketruntime.BarQuery{Instrument: eurusdID(t), Interval: marketdata.H1, Range: fixtureSpan(t)})
	require.NoError(t, err)
