...
func newSPYFixtureManager(t *testing.T) *marketruntime.Manager {
	t.Helper()
	...
	plan, err := mgr.Plan(ctx, marketruntime.BarQuery{Instrument: spyID(t), Interval: marketdata.D1, Range: spyFixtureSpan(t)})
	require.NoError(t, err)
...
func (s *spyEnterOnceStrategy) OnBar(ctx context.Context, event strategy.BarEvent, view strategy.View) ([]runtimeorder.Intent, error) {
	if s.entered {
		return nil, nil
	}
	s.entered = true
