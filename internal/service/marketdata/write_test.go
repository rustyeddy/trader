...
func newTestManagerAndServiceWithOANDA(t *testing.T, baseURL, rawRoot string) (*marketruntime.Manager, *svc.Service) {
	t.Helper()
...
// via logging.Capture.
func newTestManagerAndServiceWithOANDAAndLogger(t *testing.T, baseURL, rawRoot string, logger *slog.Logger) (*marketruntime.Manager, *svc.Service) {
	t.Helper()
...
func TestSync_DownloadsMissingRawPartition(t *testing.T) {
	...
	require.Equal(t, marketruntime.ActionDownloadRaw, resp.Plan.Actions[0].Kind)
...
func TestSync_SkipsNonDownloadActions(t *testing.T) {
	...
	require.Equal(t, marketruntime.ActionNormalizeCanonical, resp.Plan.Actions[0].Kind)
	require.Empty(t, resp.Result.Downloaded)
	require.Len(t, resp.Result.Skipped, 1)
	require.Equal(t, marketruntime.ActionNormalizeCanonical, resp.Result.Skipped[0].Action.Kind)
}
