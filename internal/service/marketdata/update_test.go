...
func TestUpdate_FullPipelineFromMissingRaw(t *testing.T) {
	...
	require.Equal(t, marketruntime.ActionDownloadRaw, resp.InitialPlan.Actions[0].Kind)
...
func TestUpdate_SkipsSyncWhenRawAlreadyPresent(t *testing.T) {
	...
	require.Equal(t, marketruntime.ActionNormalizeCanonical, resp.InitialPlan.Actions[0].Kind)
...
func TestUpdate_FailsExplicitlyOnUnresolvedRepairRaw(t *testing.T) {
	...
	require.Equal(t, marketruntime.ActionRepairRaw, resp.FinalPlan.Actions[0].Kind)
}
