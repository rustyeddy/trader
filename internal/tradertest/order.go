...
func NewOrder(p OrderParams) (runtimeorder.Order, error) {
	...
	return runtimeorder.NewOrder(runtimeorder.Order{
		Request:            p.Request,
...
func MustNewOrder(p OrderParams) runtimeorder.Order {
	o, err := NewOrder(p)
	if err != nil {
		panic(err)
	}
