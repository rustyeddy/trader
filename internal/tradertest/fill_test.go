...
func TestNewFillForPartialQuantity(t *testing.T) {
	...
	applied, err := runtimeorder.ApplyFill(o, f)
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusPartiallyFilled, applied.Status)
}
...
func TestNewFillForAppliesCleanly(t *testing.T) {
	...
	filled, err := runtimeorder.ApplyFill(o, f)
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusFilled, filled.Status)
}
