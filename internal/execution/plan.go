...
func checkPlanInput(in PlanInput) (runtimeorder.Intent, error) {
	validIntent, err := runtimeorder.NewIntent(in.Intent)
	if err != nil {
		return runtimeorder.Intent{}, fmt.Errorf("%w: intent: %v", ErrInvalidPlanInput, err)
	}
	if in.Listing.InstrumentID().IsZero() {
		return runtimeorder.Intent{}, fmt.Errorf("%w: listing must be constructed", ErrInvalidPlanInput)
	}
	if !in.Listing.InstrumentID().Equal(validIntent.Instrument) {
		return runtimeorder.Intent{}, fmt.Errorf("%w: listing instrument %s does not match intent instrument %s",
			ErrInvalidPlanInput, in.Listing.InstrumentID(), validIntent.Instrument)
	}
	if in.Account.AccountID().IsZero() {
		return runtimeorder.Intent{}, fmt.Errorf("%w: account must be constructed", ErrInvalidPlanInput)
	}
	if !strings.EqualFold(in.Listing.Provider(), in.Account.Broker()) {
		return runtimeorder.Intent{}, fmt.Errorf("%w: listing provider %q does not match account broker %q",
	...
	case requireQuantity && in.Quantity == nil:
		return runtimeorder.Intent{}, fmt.Errorf("%w: quantity must be set for %v", ErrInvalidPlanInput, validIntent.Kind)
	case requireQuantity && in.Quantity.IsZero():
		return runtimeorder.Intent{}, fmt.Errorf("%w: quantity must be positive for %v", ErrInvalidPlanInput, validIntent.Kind)
	case !requireQuantity && in.Quantity != nil:
		return runtimeorder.Intent{}, fmt.Errorf("%w: quantity must not be set for %v", ErrInvalidPlanInput, validIntent.Kind)
	}
