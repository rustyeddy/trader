...
func TestEventReader_PollDetectsCancelSettlement(t *testing.T) {
	...
	cancelResult, err := acc.Cancel(context.Background(), runtimeorder.CancelRequest{
		OrderID:  orderID,
		Metadata: id.Metadata{EventID: id.MustParseEventID("evt_01ARZ3NDEKTSV4RRFFQ69G5FAV")},
	})
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusPendingCancel, cancelResult.Status)
	...
	ev2, err := reader.Next(context.Background())
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusPendingCancel, ev2.Order.Status)
	...
	ev3, err := reader.Next(context.Background())
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusCanceled, ev3.Order.Status)
}
...
func TestEventReader_SynchronousFillEmitsFillEvent(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, brokerpkg.EventKindOrder, ev1.Kind)
	assert.Equal(t, runtimeorder.StatusFilled, ev1.Order.Status)
