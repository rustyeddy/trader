...
func (h *accountHandle) Snapshot(ctx context.Context) (account.Snapshot, error) {
	...
	positions := make([]runtimeorder.Position, 0, len(wirePositions))
	...
	openOrders := make([]runtimeorder.Order, 0, len(wireOpenOrders))
...
func (h *accountHandle) Submit(ctx context.Context, req runtimeorder.Request) (runtimeorder.Order, error) {
	...
	if err != nil {
		if errors.Is(err, ErrOrderRejected) {
			rejected, buildErr := runtimeorder.NewOrder(runtimeorder.Order{
				Request: req,
			...
			if buildErr != nil {
				return runtimeorder.Order{}, fmt.Errorf("alpaca: submit: build rejected order: %w", buildErr)
			}
			return rejected, nil
		}
		return runtimeorder.Order{}, fmt.Errorf("alpaca: submit: %w", err)
	}
	...
	if _, err := h.broker.corr.appendEvent(func(sequence uint64) (brokerpkg.Event, error) {
		...
		return runtimeorder.Order{}, fmt.Errorf("alpaca: submit: record event: %w", err)
	}
...
func buildOrderEvent(deps Deps, o runtimeorder.Order, causationID id.EventID, sequence uint64) (brokerpkg.Event, error) {
