...
func typeToWire(t runtimeorder.Type) (string, error) {
	if t != runtimeorder.Market {
		return "", fmt.Errorf("%w: order type %v is not supported by this adapter", brokerpkg.ErrUnsupported, t)
	}
	return "market", nil
}

func typeFromWire(s string) runtimeorder.Type {
	switch s {
	case "market":
		return runtimeorder.Market
	case "limit":
		return runtimeorder.Limit
	case "stop":
		return runtimeorder.Stop
	case "stop_limit":
		return runtimeorder.StopLimit
	default:
		return 0
	}
}
...
func tifToWire(f runtimeorder.TimeInForce) (string, error) {
	switch f {
	case runtimeorder.GTC:
		return "gtc", nil
	case runtimeorder.DAY:
		return "day", nil
	case runtimeorder.IOC:
		return "ioc", nil
	case runtimeorder.FOK:
		return "fok", nil
	default:
...
func tifFromWire(s string) runtimeorder.TimeInForce {
	switch s {
	case "gtc":
		return runtimeorder.GTC
	case "day":
		return runtimeorder.DAY
	case "ioc":
		return runtimeorder.IOC
	case "fok":
		return runtimeorder.FOK
	default:
		return 0
	}
}
...
func statusFromWire(s string) runtimeorder.Status {
	switch s {
	case "new", "accepted", "pending_new", "accepted_for_bidding":
		return runtimeorder.StatusWorking
	case "partially_filled":
		return runtimeorder.StatusPartiallyFilled
	case "filled":
		return runtimeorder.StatusFilled
	case "pending_cancel":
		return runtimeorder.StatusPendingCancel
	case "canceled", "replaced":
		return runtimeorder.StatusCanceled
	case "pending_replace":
		return runtimeorder.StatusPendingReplace
	case "rejected":
		return runtimeorder.StatusRejected
	case "expired", "done_for_day":
		return runtimeorder.StatusExpired
	default:
...
func requestToWireSubmit(req runtimeorder.Request) (wireOrderSubmit, error) {
...
func wireOrderToOrder(w wireOrder, resolver instrument.Resolver, providerName string, accountID id.AccountID, ids *id.Generator) (runtimeorder.Order, error) {
	orderID, err := id.ParseOrderID(w.ClientOrderID)
	if err != nil {
		return runtimeorder.Order{}, fmt.Errorf("alpaca: parse client_order_id %q: %w", w.ClientOrderID, err)
	}
	...
	if err != nil {
		return runtimeorder.Order{}, fmt.Errorf("alpaca: parse limit_price: %w", err)
	}
	stopPrice, err := optionalPrice(w.StopPrice)
	if err != nil {
		return runtimeorder.Order{}, fmt.Errorf("alpaca: parse stop_price: %w", err)
	}

	qty, err := num.ParseQuantity(w.Qty)
	if err != nil {
		return runtimeorder.Order{}, fmt.Errorf("alpaca: parse qty %q: %w", w.Qty, err)
	}
	proposal := runtimeorder.Proposal{
		Listing:     listing,
		AccountID:   accountID,
		Side:        side,
		Type:        typ,
	...
	if err != nil {
		return runtimeorder.Order{}, fmt.Errorf("alpaca: reconstruct request: %w", err)
	}

	status := statusFromWire(w.Status)

	o := runtimeorder.Order{
		Request:       req,
		BrokerOrderID: w.ID,
		Status:        status,
	}
	if status == runtimeorder.StatusPendingCancel || status == runtimeorder.StatusPendingReplace {
		pendingID, err := id.GenerateEventID(ids)
		if err != nil {
			return runtimeorder.Order{}, fmt.Errorf("alpaca: synthesize pending command id: %w", err)
		}
	...
	if requiresAcceptedQuantity(status) {
		...
		if err != nil {
			return runtimeorder.Order{}, fmt.Errorf("alpaca: parse filled_qty %q: %w", w.FilledQty, err)
		}
		...
		if err != nil {
			return runtimeorder.Order{}, fmt.Errorf("alpaca: parse filled_avg_price: %w", err)
		}
		o.AvgFillPrice = avgFillPrice
	}
	if status == runtimeorder.StatusRejected {
		o.Rejection = &runtimeorder.Rejection{Reason: runtimeorder.ReasonUnknown, Detail: "alpaca reported status \"" + w.Status + "\""}
	}

	built, err := runtimeorder.NewOrder(o)
	if err != nil {
		return runtimeorder.Order{}, fmt.Errorf("alpaca: build order: %w", err)
	}
	return built, nil
}
...
func requiresAcceptedQuantity(s runtimeorder.Status) bool {
	switch s {
	case runtimeorder.StatusPendingSubmit, runtimeorder.StatusRejected:
		return false
	default:
		return true
	}
...
// order.Position.
func wirePositionToPosition(w wirePosition, resolver instrument.Resolver, providerName string, accountID id.AccountID) (runtimeorder.Position, error) {
	listing, err := resolveListing(resolver, providerName, w.Symbol)
	if err != nil {
	...
	case "short":
		side = order.Short
	default:
		return runtimeorder.Position{}, fmt.Errorf("alpaca: unrecognized position side %q for %s", w.Side, w.Symbol)
	}
	qty, err := num.ParseQuantity(w.Qty)
	if err != nil {
		return runtimeorder.Position{}, fmt.Errorf("alpaca: parse position qty %q: %w", w.Qty, err)
	}
	avgPrice, err := num.ParsePrice(w.AvgEntryPrice)
	if err != nil {
		return runtimeorder.Position{}, fmt.Errorf("alpaca: parse avg_entry_price %q: %w", w.AvgEntryPrice, err)
	}
	...
	if err != nil {
		return runtimeorder.Position{}, fmt.Errorf("alpaca: build position for %s: %w", w.Symbol, err)
	}
	return pos, nil
}
...
func wireAccountToSnapshotParams(w wireAccount, positions []runtimeorder.Position, openOrders []runtimeorder.Order, unrealizedPnL num.Money, accountID id.AccountID, broker string, asOf time.Time) (account.SnapshotParams, error) {
