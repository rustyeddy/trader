...
func TestAccountHandle_Submit_ClosedBroker(t *testing.T) {
	...
	_, err = acc.Submit(context.Background(), runtimeorder.Request{})
...
func TestAccountHandle_Submit_TransportError(t *testing.T) {
	...
	req, err := runtimeorder.NewRequest(runtimeorder.Proposal{
		Listing: listing, AccountID: accountID, Side: order.Buy, Type: runtimeorder.Market,
...
func TestAccountHandle_Cancel_ClosedBroker(t *testing.T) {
	...
	_, err = acc.Cancel(context.Background(), runtimeorder.CancelRequest{OrderID: id.MustParseOrderID("ord_01ARZ3NDEKTSV4RRFFQ69G5FAV")})
	assert.ErrorIs(t, err, brokerpkg.ErrClosed)
}
...
func TestAccountHandle_Cancel_ContextCanceled(t *testing.T) {
	...
	cancel()
	_, err = acc.Cancel(ctx, runtimeorder.CancelRequest{
		OrderID:  id.MustParseOrderID("ord_01ARZ3NDEKTSV4RRFFQ69G5FAV"),
...
func TestAccountHandle_Replace_ClosedBroker(t *testing.T) {
	...
	_, err = acc.Replace(context.Background(), runtimeorder.ReplaceRequest{OrderID: id.MustParseOrderID("ord_01ARZ3NDEKTSV4RRFFQ69G5FAV"), NewQuantity: &newQty})
	assert.ErrorIs(t, err, brokerpkg.ErrClosed)
}
...
func TestAccountHandle_Replace_InvalidRequest(t *testing.T) {
	...
	_, err = acc.Replace(context.Background(), runtimeorder.ReplaceRequest{OrderID: id.MustParseOrderID("ord_01ARZ3NDEKTSV4RRFFQ69G5FAV")})
	require.Error(t, err)
}
...
func TestAccountHandle_Replace_ContextCanceled(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	_, err = acc.Replace(ctx, runtimeorder.ReplaceRequest{
...
func TestStatusFromWire_ReplacedIsTerminalNotPending(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.StatusCanceled, statusFromWire("replaced"))
	assert.Equal(t, runtimeorder.StatusPendingReplace, statusFromWire("pending_replace"))
}
...
func TestEventReader_Poll_SkipsForeignOrder(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.StatusFilled, ev2.Order.Status)
