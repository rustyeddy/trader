...
func TestTypeTranslation(t *testing.T) {
	s, err := typeToWire(runtimeorder.Market)
	require.NoError(t, err)
	assert.Equal(t, "market", s)

	_, err = typeToWire(runtimeorder.Limit)
	assert.ErrorIs(t, err, brokerpkg.ErrUnsupported)

	assert.Equal(t, runtimeorder.Limit, typeFromWire("limit"))
	assert.Equal(t, runtimeorder.Stop, typeFromWire("stop"))
	assert.Equal(t, runtimeorder.StopLimit, typeFromWire("stop_limit"))
	assert.Zero(t, typeFromWire("bogus"))
}

func TestTimeInForceTranslation(t *testing.T) {
	for _, tif := range []runtimeorder.TimeInForce{runtimeorder.GTC, runtimeorder.DAY, runtimeorder.IOC, runtimeorder.FOK} {
		s, err := tifToWire(tif)
		require.NoError(t, err)
...
func TestStatusFromWire(t *testing.T) {
	cases := map[string]runtimeorder.Status{
		"new":              runtimeorder.StatusWorking,
		"accepted":         runtimeorder.StatusWorking,
		"pending_new":      runtimeorder.StatusWorking,
		"partially_filled": runtimeorder.StatusPartiallyFilled,
		"filled":           runtimeorder.StatusFilled,
		"pending_cancel":   runtimeorder.StatusPendingCancel,
		"canceled":         runtimeorder.StatusCanceled,
		"pending_replace":  runtimeorder.StatusPendingReplace,
		"replaced":         runtimeorder.StatusCanceled,
		"rejected":         runtimeorder.StatusRejected,
		"expired":          runtimeorder.StatusExpired,
		"done_for_day":     runtimeorder.StatusExpired,
		"something_new":    runtimeorder.StatusUnknown,
	}
