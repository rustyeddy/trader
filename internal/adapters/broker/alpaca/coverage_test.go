...
func TestTifToWire_Invalid(t *testing.T) {
	_, err := tifToWire(runtimeorder.TimeInForce(99))
	require.Error(t, err)
...
func TestRequestToWireSubmit_InvalidSide(t *testing.T) {
	...
	proposal := runtimeorder.Proposal{
		...
		Side: order.Buy, Type: runtimeorder.Market, TimeInForce: runtimeorder.DAY, Quantity: num.MustParseQuantity("1"),
	}
	req, err := runtimeorder.NewRequest(proposal, id.MustParseOrderID("ord_01ARZ3NDEKTSV4RRFFQ69G5FAV"))
...
func TestEmitFillIfIncreased_MissingAvgPriceErrors(t *testing.T) {
	...
	o, err := runtimeorder.NewOrder(runtimeorder.Order{
		...
		Status:           runtimeorder.StatusFilled,
...
func mustRequest(t *testing.T, listing instrument.Listing, accountID id.AccountID) runtimeorder.Request {
	t.Helper()
