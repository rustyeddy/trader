...
func mustRequest(t *testing.T, gen *id.Generator, accountID id.AccountID) runtimeorder.Request {
	t.Helper()
	...
	require.NoError(t, err)
	req, err := runtimeorder.NewRequest(proposal, mustOrderID(t, gen))
	require.NoError(t, err)
	return req
}
...
func mustMarketRequest(t *testing.T, gen *id.Generator, accountID id.AccountID, side order.Side, quantity string) runtimeorder.Request {
	t.Helper()
	return mustMarketRequestFor(t, gen, accountID, mustEurUsdListing(t), side, quantity)
}

func mustMarketRequestFor(t *testing.T, gen *id.Generator, accountID id.AccountID, listing instrument.Listing, side order.Side, quantity string) runtimeorder.Request {
	t.Helper()
	proposal, err := runtimeorder.NewProposal(runtimeorder.Proposal{
		Listing:     listing,
		AccountID:   accountID,
...
func mustLimitRequest(t *testing.T, gen *id.Generator, accountID id.AccountID, side order.Side, quantity, limitPrice string) runtimeorder.Request {
	t.Helper()
	return mustLimitRequestFor(t, gen, accountID, mustEurUsdListing(t), side, quantity, limitPrice)
}

func mustLimitRequestFor(t *testing.T, gen *id.Generator, accountID id.AccountID, listing instrument.Listing, side order.Side, quantity, limitPrice string) runtimeorder.Request {
	t.Helper()
...
func mustStopRequest(t *testing.T, gen *id.Generator, accountID id.AccountID, side order.Side, quantity, stopPrice string) runtimeorder.Request {
	t.Helper()
	return mustStopRequestFor(t, gen, accountID, mustEurUsdListing(t), side, quantity, stopPrice)
}

func mustStopRequestFor(t *testing.T, gen *id.Generator, accountID id.AccountID, listing instrument.Listing, side order.Side, quantity, stopPrice string) runtimeorder.Request {
	t.Helper()
...
func TestBrokerCloseRejectsFurtherUse(t *testing.T) {
	...
	_, err = acc.Cancel(ctx, runtimeorder.CancelRequest{OrderID: mustOrderID(t, deps.IDs), Metadata: id.Metadata{EventID: mustEventID(t, deps.IDs)}})
	require.ErrorIs(t, err, brokerpkg.ErrClosed)

	qty := num.MustParseQuantity("1")
	_, err = acc.Replace(ctx, runtimeorder.ReplaceRequest{OrderID: mustOrderID(t, deps.IDs), NewQuantity: &qty, Metadata: id.Metadata{EventID: mustEventID(t, deps.IDs)}})
	require.ErrorIs(t, err, brokerpkg.ErrClosed)
...
func TestAccountSubmitAcceptsOrderIntoWorkingWithoutFill(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusWorking, o.Status)
...
func TestAccountSubmitRejectsInvalidRequest(t *testing.T) {
	...
	_, err = acc.Submit(ctx, runtimeorder.Request{}) // zero value fails order.NewRequest's validation chain
	require.Error(t, err)
}
...
func TestAccountSubmitLimitOrderAcceptsRequestedPrice(t *testing.T) {
	...
	require.NoError(t, err)
	req, err := runtimeorder.NewRequest(proposal, mustOrderID(t, deps.IDs))
	require.NoError(t, err)
...
func TestAccountSubmitMarketBuyFillsImmediatelyAndOpensLongPosition(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.StatusWorking, events[0].Order.Status)
	...
	assert.Equal(t, runtimeorder.StatusFilled, events[2].Order.Status)
...
func TestAccountSubmitMarketOrderIsIdempotentAfterFill(t *testing.T) {
	...
	second, err := acc.Submit(ctx, req)
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusFilled, second.Status)
...
func TestAccountSubmitLeavesNoStateWhenEventGenerationFails(t *testing.T) {
	...
	require.NoError(t, err)
	req, err := runtimeorder.NewRequest(proposal, mustOrderID(t, workingGen))
	require.NoError(t, err)
...
func TestCloneOrderClonesEveryPointerAndSliceField(t *testing.T) {
	...
	original, err := runtimeorder.NewOrder(runtimeorder.Order{
		Request:              req,
...
func TestCloneOrderClonesRejection(t *testing.T) {
	...
	original, err := runtimeorder.NewOrder(runtimeorder.Order{
		...
		Rejection: &runtimeorder.Rejection{
			Reason:     runtimeorder.ReasonInsufficientMargin,
