...
func TestObserveMark_NeverTriggersRestingOrders(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, runtimeorder.StatusWorking, limitOrder.Status)
	...
	assert.Equal(t, runtimeorder.StatusWorking, snap.OpenOrders()[0].Status, "ObserveMark must never evaluate or fill resting order triggers")
}
