...
func (h *accountHandle) Cancel(ctx context.Context, req runtimeorder.CancelRequest) (runtimeorder.CancelResult, error) {
	...
	if o.Status == runtimeorder.StatusCanceled {
		return runtimeorder.NewCancelResult(runtimeorder.CancelResult{OrderID: req.OrderID, Status: runtimeorder.StatusCanceled, Metadata: resultMeta})
	}

	if o.Status != runtimeorder.StatusWorking && o.Status != runtimeorder.StatusPartiallyFilled {
...
func (h *accountHandle) Replace(ctx context.Context, req runtimeorder.ReplaceRequest) (runtimeorder.ReplaceResult, error) {
	...
	_, validationErr := runtimeorder.NewOrder(proposed)

	var result runtimeorder.ReplaceResult
	...
	replaced, err := runtimeorder.ApplyReplaceResult(pending, req, result)
	if err != nil {
		return runtimeorder.ReplaceResult{}, err
	}
