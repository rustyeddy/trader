...
func mustReduceOnlyMarketRequestFor(t *testing.T, gen *id.Generator, accountID id.AccountID, listing instrument.Listing, side order.Side, quantity string) runtimeorder.Request {
	t.Helper()
...
func mustReduceOnlyStopRequestFor(t *testing.T, gen *id.Generator, accountID id.AccountID, listing instrument.Listing, side order.Side, quantity, stopPrice string) runtimeorder.Request {
	t.Helper()
...
func openLongPosition(t *testing.T, ctx context.Context, acc interface {
	Submit(context.Context, runtimeorder.Request) (runtimeorder.Order, error)
...
func TestReduceOnly_SellAgainstFlatNeverOpensShort(t *testing.T) {
	...
	o, err := acc.Submit(ctx, mustReduceOnlyMarketRequestFor(t, deps.IDs, accountID, mustEurUsdListing(t), order.Sell, "1000"))
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusCanceled, o.Status, "a reduce-only order with nothing to reduce must be canceled, not filled")
...
func TestReduceOnly_SellAgainstShortNeverIncreasesShort(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusCanceled, o.Status)
...
func TestReduceOnly_OversizedCannotReversePosition(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusCanceled, o.Status, "the unfillable remainder must be canceled atomically, not left partially filled and live")
...
func TestReduceOnly_StaleProtectiveStopCannotFillIntoFlat(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, runtimeorder.StatusWorking, stopOrder.Status)
	...
	require.NoError(t, err)
	require.Equal(t, runtimeorder.StatusFilled, exitOrder.Status)
	...
	assert.Equal(t, runtimeorder.StatusCanceled, final.Status, "the stale protective stop must be canceled, not left dangling or filled")
	...
	for {
		...
		case runtimeorder.StatusPendingCancel:
			pendingEvt = e
