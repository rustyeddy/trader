...
func mustCancelRequest(t *testing.T, gen *id.Generator, orderID id.OrderID) runtimeorder.CancelRequest {
	t.Helper()
	req, err := runtimeorder.NewCancelRequest(runtimeorder.CancelRequest{OrderID: orderID, Metadata: id.Metadata{EventID: mustEventID(t, gen)}})
	require.NoError(t, err)
	return req
}

func mustReplaceRequest(t *testing.T, gen *id.Generator, orderID id.OrderID, newQuantity string) runtimeorder.ReplaceRequest {
	t.Helper()
	qty := num.MustParseQuantity(newQuantity)
	req, err := runtimeorder.NewReplaceRequest(runtimeorder.ReplaceRequest{OrderID: orderID, NewQuantity: &qty, Metadata: id.Metadata{EventID: mustEventID(t, gen)}})
	require.NoError(t, err)
	return req
}
...
func TestCancelWorkingOrderSucceeds(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusCanceled, result.Status)
	...
	assert.Equal(t, runtimeorder.StatusPendingCancel, events[1].Order.Status)
	...
	assert.Equal(t, runtimeorder.StatusCanceled, events[2].Order.Status)
...
func TestCancelAlreadyCanceledOrderIsIdempotent(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, runtimeorder.StatusCanceled, first.Status)

	second, err := acc.Cancel(ctx, mustCancelRequest(t, deps.IDs, req.OrderID))
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusCanceled, second.Status)
...
func TestCancelRejectsInvalidRequest(t *testing.T) {
	...
	_, err = acc.Cancel(ctx, runtimeorder.CancelRequest{}) // zero OrderID
	require.Error(t, err)
}
...
func TestReplaceWorkingOrderQuantitySucceeds(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, runtimeorder.StatusWorking, result.Status)
	assert.Nil(t, result.Rejection)
	...
	require.Equal(t, runtimeorder.StatusPendingReplace, events[1].Order.Status)
	...
	require.Equal(t, runtimeorder.StatusWorking, events[2].Order.Status)
...
func TestReplaceInvalidQuantityIsDeclined(t *testing.T) {
	...
	assertNoMoreEventsSoon(t, reader)
	require.Equal(t, runtimeorder.StatusWorking, events[2].Order.Status, "a declined replace still reports the resolved (unchanged) status via its own event")
}
...
func TestReplaceRejectsInvalidRequest(t *testing.T) {
	...
	_, err = acc.Replace(ctx, runtimeorder.ReplaceRequest{}) // zero OrderID, no new values
	require.Error(t, err)
}
...
func TestCancelRejectsZeroMetadataEventID(t *testing.T) {
	...
	_, err = acc.Submit(ctx, working)
	require.NoError(t, err)
	_, err = acc.Cancel(ctx, runtimeorder.CancelRequest{OrderID: working.OrderID})
	require.Error(t, err)

	filled := mustMarketRequest(t, deps.IDs, accountID, order.Buy, "1000")
	_, err = acc.Submit(ctx, filled)
	require.NoError(t, err)
	_, err = acc.Cancel(ctx, runtimeorder.CancelRequest{OrderID: filled.OrderID})
	require.Error(t, err)
}
...
func TestReplaceRejectsZeroMetadataEventID(t *testing.T) {
	...
	_, err = acc.Submit(ctx, working)
	require.NoError(t, err)
	_, err = acc.Replace(ctx, runtimeorder.ReplaceRequest{OrderID: working.OrderID, NewQuantity: &qty})
	require.Error(t, err)

	filled := mustMarketRequest(t, deps.IDs, accountID, order.Buy, "1000")
	_, err = acc.Submit(ctx, filled)
	require.NoError(t, err)
	_, err = acc.Replace(ctx, runtimeorder.ReplaceRequest{OrderID: filled.OrderID, NewQuantity: &qty})
	require.Error(t, err)
}
