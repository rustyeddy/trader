...
func TestVerticalSlice_FullScenarioProducesExpectedState(t *testing.T) {
	...
	t.Run("account A: event stream reflects the full order lifecycle", func(t *testing.T) {
		...
		assert.Equal(t, runtimeorder.StatusWorking, events[0].Order.Status, "the market buy's own accept event")
		...
		for _, e := range events {
			...
			if e.Kind == brokerpkg.EventKindOrder && e.Order.Status == runtimeorder.StatusCanceled {
				canceledCount++
			}
		}
		...
		assert.Equal(t, runtimeorder.StatusFilled, last.Order.Status)
	})
