...
func TestToWireAccountSnapshot_EveryPositionSide(t *testing.T) {
	...
	short, err := runtimeorder.NewPosition(runtimeorder.Position{
		AccountID: acctID,
	...
	flat, err := runtimeorder.NewPosition(runtimeorder.Position{
		AccountID: acctID,
	...
	snap, err := account.NewSnapshot(account.SnapshotParams{
		...
		Positions:       []runtimeorder.Position{short, flat},
		Equity:          zero,
		BuyingPower:     zero,
		MarginUsed:      zero,
