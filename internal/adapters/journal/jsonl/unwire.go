...
func fromIntentWire(w intentWire) (runtimeorder.Intent, error) {
...
func fromProposalWire(w proposalWire) (runtimeorder.Proposal, error) {
...
func fromRequestWire(w requestWire) (runtimeorder.Request, error) {
	proposal, err := fromProposalWire(w.Proposal)
	if err != nil {
		return runtimeorder.Request{}, err
	}
	return runtimeorder.Request{Proposal: proposal, OrderID: w.OrderID}, nil
}

func fromReplaceRequestWire(w replaceRequestWire) (runtimeorder.ReplaceRequest, error) {
	return runtimeorder.NewReplaceRequest(runtimeorder.ReplaceRequest{
		OrderID:       w.OrderID,
...
func fromOrderWire(w orderWire) (runtimeorder.Order, error) {
...
func fromFillWire(w fillWire) (runtimeorder.Fill, error) {
	listing, err := fromListingWire(w.Listing)
	if err != nil {
...
func fromPositionWire(w positionWire) (runtimeorder.Position, error) {
	listing, err := fromListingWire(w.Listing)
	if err != nil {
	...
	return runtimeorder.Position{AccountID: w.AccountID, Listing: listing, Side: side, Quantity: w.Quantity, AvgPrice: w.AvgPrice}, nil
}

func fromAccountWire(w accountWire) (account.Snapshot, error) {
	positions := make([]runtimeorder.Position, 0, len(w.Positions))
...
func fromTradeWire(w tradeWire) (runtimeorder.Trade, error) {
	listing, err := fromListingWire(w.Listing)
	if err != nil {
		return runtimeorder.Trade{}, err
	}
	side, err := parsePositionSide(w.Side)
	if err != nil {
		return runtimeorder.Trade{}, err
	}
	return runtimeorder.Trade{
		AccountID:    w.AccountID,
		Listing:      listing,
		Side:         side,
