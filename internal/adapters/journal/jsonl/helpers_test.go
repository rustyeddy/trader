...
func mustIntent(t *testing.T) runtimeorder.Intent {
	t.Helper()
...
func mustProposal(t *testing.T) runtimeorder.Proposal {
	t.Helper()
	p, err := runtimeorder.NewProposal(runtimeorder.Proposal{
...
func mustRequest(t *testing.T) runtimeorder.Request {
	t.Helper()
	r, err := runtimeorder.NewRequest(mustProposal(t), mustOrderID(t))
	require.NoError(t, err)
	return r
}

func mustReplaceRequest(t *testing.T) runtimeorder.ReplaceRequest {
	t.Helper()
	sp := num.MustParsePrice("1.06000")
	r, err := runtimeorder.NewReplaceRequest(runtimeorder.ReplaceRequest{
...
func mustWorkingOrder(t *testing.T) runtimeorder.Order {
	t.Helper()
	return mustWorkingOrderFor(t, mustAccountID(t))
}

func mustWorkingOrderFor(t *testing.T, accountID id.AccountID) runtimeorder.Order {
	t.Helper()
	...
	require.NoError(t, err)
	req, err := runtimeorder.NewRequest(proposal, mustOrderID(t))
	require.NoError(t, err)
...
func mustFill(t *testing.T) runtimeorder.Fill {
	t.Helper()
...
func mustSnapshot(t *testing.T) account.Snapshot {
	...
	snap, err := account.NewSnapshot(account.SnapshotParams{
		...
		OpenOrders:      []runtimeorder.Order{mustWorkingOrderFor(t, accountID)},
	})
	require.NoError(t, err)
	return snap
}

func mustRejectedOrder(t *testing.T) runtimeorder.Order {
	t.Helper()
	proposal, err := runtimeorder.NewProposal(runtimeorder.Proposal{
		Listing:     mustEurUsdListing(t),
		AccountID:   mustAccountID(t),
...
func mustTrade(t *testing.T) runtimeorder.Trade {
	t.Helper()
	usd := num.MustParseCurrency("USD")
	tr, err := runtimeorder.NewTrade(runtimeorder.Trade{
		AccountID:    mustAccountID(t),
		Listing:      mustEurUsdListing(t),
		Side:         order.Long,
