...
func (s *enterThenExitStrategy) OnBar(ctx context.Context, ev strategy.BarEvent, view strategy.View) ([]runtimeorder.Intent, error) {
	...
	if !s.exited[key] {
		...
		return []runtimeorder.Intent{ex}, nil
	}
	return nil, nil
}
...
func newDeterminismRunnerParams(t *testing.T, mgr *marketruntime.Manager, seed1, seed2 uint64, strat strategy.Strategy) (backtest.RunnerParams, *capturingRecorder) {
	t.Helper()
