...
func (f tradesFixture) fill(t *testing.T, listing instrument.Listing, side order.Side, price, quantity string, commission string, ts time.Time) runtimeorder.Fill {
	t.Helper()
	fillID, err := id.GenerateFillID(f.ids)
	require.NoError(t, err)

	params := runtimeorder.Fill{
		FillID:    fillID,
		OrderID:   f.orderID,
	...
	fill, err := runtimeorder.NewFill(params)
	require.NoError(t, err)
	return fill
}
...
func TestDeriveTrades_SimpleOpenAndClose(t *testing.T) {
	...
	set, err := backtest.DeriveTrades([]runtimeorder.Fill{open, closeFill})
	require.NoError(t, err)
...
func TestDeriveTrades_PartialReduceThenClose(t *testing.T) {
	...
	set, err := backtest.DeriveTrades([]runtimeorder.Fill{open, reduce, closeRest})
	require.NoError(t, err)
	require.Empty(t, set.Open)
	require.Len(t, set.Closed, 1)
...
func TestDeriveTrades_MultiLotAveragingEntry(t *testing.T) {
	...
	set, err := backtest.DeriveTrades([]runtimeorder.Fill{first, second, closeAll})
	require.NoError(t, err)
	require.Len(t, set.Closed, 1)
...
func TestDeriveTrades_Reversal(t *testing.T) {
	...
	set, err := backtest.DeriveTrades([]runtimeorder.Fill{open, reverse})
	require.NoError(t, err)
	require.Len(t, set.Closed, 1)
...
func TestDeriveTrades_MultiInstrumentInterleaving(t *testing.T) {
	...
	set, err := backtest.DeriveTrades([]runtimeorder.Fill{eOpen, gOpen, eClose, gClose})
	require.NoError(t, err)
	require.Empty(t, set.Open)
	require.Len(t, set.Closed, 2)

	byListing := map[string]runtimeorder.Trade{}
...
func TestDeriveTrades_StillOpenAtEndOfStream(t *testing.T) {
	...
	set, err := backtest.DeriveTrades([]runtimeorder.Fill{open})
	require.NoError(t, err)
...
func TestDeriveTrades_OpenOrderingIsDeterministicForSameTimestamp(t *testing.T) {
	...
	var want []string
	for range 20 {
		set, err := backtest.DeriveTrades([]runtimeorder.Fill{gOpen, eOpen})
		require.NoError(t, err)
...
func TestDeriveTrades_RejectsMixedAccounts(t *testing.T) {
	...
	_, err := backtest.DeriveTrades([]runtimeorder.Fill{a, b})
