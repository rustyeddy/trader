		...
		Failures: []FailedRequirement{
			{
				...
				Err:         fmt.Errorf("%w: no coverage for [2024-01-16T22:00:00Z, 2024-01-17T22:00:00Z)", marketruntime.ErrDataUnavailable),
			},
		},
	}
	assert.Contains(t, err.Error(), "no coverage for")
	assert.ErrorIs(t, err, marketruntime.ErrDataUnavailable)
}
...
func TestCoverageErrorUnwrapDoesNotMatchUnrelatedErrors(t *testing.T) {
	err := &CoverageError{
		Failures: []FailedRequirement{
			{
				Requirement: strategy.DataRequirement{Instrument: eurusdIDForTest(t), Interval: marketdata.H1},
				Err:         fmt.Errorf("%w: gap", marketruntime.ErrDataUnavailable),
			},
		},
	}
	assert.NotErrorIs(t, err, marketruntime.ErrInconsistentData)
}
