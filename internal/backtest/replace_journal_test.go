...
func (s *enterThenRatchetStopStrategy) OnBar(ctx context.Context, ev strategy.BarEvent, view strategy.View) ([]runtimeorder.Intent, error) {
	...
	for _, o := range view.Account().OpenOrders() {
		if o.Request.Type == runtimeorder.Stop && o.Request.Listing.InstrumentID().Equal(s.eurusd) && !o.Status.Terminal() {
			hasRestingStop = true
		}
	}
