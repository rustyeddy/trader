...
func publishBothInstrumentsFixture(t *testing.T, mgr *marketruntime.Manager) {
	t.Helper()
...
func TestRunner_PropagatesComponentErrors(t *testing.T) {
	...
	_, err = runner.Run(context.Background())
	require.ErrorIs(t, err, marketruntime.ErrDataUnavailable)
}
...
func (s *alwaysErrorOnBarStrategy) OnBar(ctx context.Context, ev strategy.BarEvent, view strategy.View) ([]runtimeorder.Intent, error) {
...
func TestResolverInputBuilder_Build_UnresolvableInstrumentFails(t *testing.T) {
	...
	require.NoError(t, err)
	intent, err := runtimeorder.NewIntent(runtimeorder.Intent{
		IntentID:   intentID,
...
func (s *startErrorStrategy) OnBar(ctx context.Context, ev strategy.BarEvent, view strategy.View) ([]runtimeorder.Intent, error) {
	return nil, nil
}
