...
type replayStream struct {
	...
	reader *marketruntime.BarReader
...
func NewReplay(ctx context.Context, manager *marketruntime.Manager, requirements []strategy.DataRequirement, span marketdata.TimeRange) (*Replay, error) {
	...
	for _, req := range requirements {
		reader, err := manager.Bars(ctx, marketruntime.BarQuery{
			Instrument: req.Instrument,
			Interval:   req.Interval,
			Range:      span,
		})
		if err != nil {
			if !errors.Is(err, marketruntime.ErrDataUnavailable) {
