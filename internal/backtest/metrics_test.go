...
func metricsTrade(t *testing.T, listing instrument.Listing, realizedPnL, costs string, closedAt time.Time) runtimeorder.Trade {
	t.Helper()
	return metricsTradeSide(t, listing, order.Long, realizedPnL, costs, closedAt)
}

func metricsTradeSide(t *testing.T, listing instrument.Listing, side order.PositionSide, realizedPnL, costs string, closedAt time.Time) runtimeorder.Trade {
	t.Helper()
...
func TestNewMetrics_RejectsCurrencyMismatch(t *testing.T) {
	...
	badTrade, err := runtimeorder.NewTrade(runtimeorder.Trade{
		AccountID:    mustMetricsAccountID(t),
		Listing:      listing,
		Side:         order.Long,
		EntryFillIDs: []id.FillID{mustMetricsFillID(t)},
	...
	_, err = backtest.NewMetrics(backtest.MetricsParams{
		StartingCapital: metricsUSD("10000"),
		FinalEquity:     metricsUSD("10000"),
		Trades:          []runtimeorder.Trade{badTrade},
	})
...
func TestNewMetrics_AccountFeesIsDistinctFromClosedTradeCosts(t *testing.T) {
	listing := simListing(t, "EUR", "USD", "EUR_USD")
	t0 := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
	trades := []runtimeorder.Trade{metricsTrade(t, listing, "100", "1", t0)}
...
func TestNewMetrics_RejectsTradeWithZeroClosedAt(t *testing.T) {
	listing := simListing(t, "EUR", "USD", "EUR_USD")
	stillOpen, err := runtimeorder.NewTrade(runtimeorder.Trade{
		AccountID:    mustMetricsAccountID(t),
		Listing:      listing,
		Side:         order.Long,
		EntryFillIDs: []id.FillID{mustMetricsFillID(t)},
	...
	_, err = backtest.NewMetrics(backtest.MetricsParams{
		StartingCapital: metricsUSD("10000"),
		FinalEquity:     metricsUSD("10000"),
		Trades:          []runtimeorder.Trade{stillOpen},
		AccountFees:     metricsUSD("0"),
	})
	require.ErrorIs(t, err, backtest.ErrInvalidMetrics)
}
...
func TestNewMetrics_BySideOmitsAbsentSide(t *testing.T) {
	...
	m, err := backtest.NewMetrics(backtest.MetricsParams{
		...
		Trades:          []runtimeorder.Trade{metricsTradeSide(t, eurusd, order.Long, "100", "0", t0)},
		AccountFees:     metricsUSD("0"),
	})
	require.NoError(t, err)
...
func TestNewMetrics_RejectsTradeWithFlatSide(t *testing.T) {
	...
	_, err := backtest.NewMetrics(backtest.MetricsParams{
		...
		Trades:          []runtimeorder.Trade{flatTrade},
		AccountFees:     metricsUSD("0"),
	})
	require.ErrorIs(t, err, backtest.ErrInvalidMetrics)
}
