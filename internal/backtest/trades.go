...
type TradeSet struct {
	Closed []runtimeorder.Trade
	Open   []runtimeorder.Trade
}
...
func DeriveTrades(fills []runtimeorder.Fill) (TradeSet, error) {
	var (
		accountID   id.AccountID
		haveAccount bool
		positions   = map[tradeKey]runningPosition{}
		openTrades  = map[tradeKey]*runtimeorder.Trade{}
		closed      []runtimeorder.Trade
	)
	...
	for _, fill := range fills {
		...
		transition, err := runtimeorder.ApplyFillToPosition(rp.position, rp.has, fill.AccountID, fill.Listing, currency, fill.Side, fill.Price, fill.Quantity)
		if err != nil {
		...
		case runtimeorder.TransitionOpen:
			openTrades[key] = &runtimeorder.Trade{
				AccountID:    fill.AccountID,
		...
		case runtimeorder.TransitionIncrease:
			t, ok := openTrades[key]
			if !ok {
		...
		case runtimeorder.TransitionReduce, runtimeorder.TransitionClose:
			t, ok := openTrades[key]
			if !ok {
			...
			if transition.Transition == runtimeorder.TransitionClose {
				t.ClosedAt = fill.Timestamp
				finalized, err := runtimeorder.NewTrade(*t)
				if err != nil {
		...
		case runtimeorder.TransitionReverse:
			t, ok := openTrades[key]
			if !ok {
	...
	open := make([]runtimeorder.Trade, 0, len(openTrades))
...
// function itself.
func lessOpenTrade(a, b runtimeorder.Trade) bool {
