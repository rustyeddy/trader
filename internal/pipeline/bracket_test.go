...
func mustEnterWithStopIntent(t *testing.T, ids *id.Generator, instID instrument.ID, side order.Side, stopPrice string) runtimeorder.Intent {
	t.Helper()
...
func TestPipelineSubmit_BracketEndToEndAgainstSimulator(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.Market, entry.Proposal.Type)
	assert.Equal(t, runtimeorder.StatusFilled, entry.Order.Status)
	...
	assert.Equal(t, runtimeorder.Stop, stop.Proposal.Type)
	...
	assert.Equal(t, runtimeorder.StatusWorking, stop.Order.Status)
	...
	assert.Equal(t, runtimeorder.Stop, after.OpenOrders()[0].Request.Type)
	assert.Equal(t, runtimeorder.StatusWorking, after.OpenOrders()[0].Status)
}
...
func TestPipelineSubmit_BracketStopRejectedLeavesEntryFilledAndUnprotected(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.StatusFilled, result.Bracket.Entry.Order.Status)
...
func (a notSynchronouslyFillingAccount) Submit(ctx context.Context, req runtimeorder.Request) (runtimeorder.Order, error) {
...
func (a notSynchronouslyFillingAccount) Cancel(ctx context.Context, req runtimeorder.CancelRequest) (runtimeorder.CancelResult, error) {
	return a.inner.Cancel(ctx, req)
}

func (a notSynchronouslyFillingAccount) Replace(ctx context.Context, req runtimeorder.ReplaceRequest) (runtimeorder.ReplaceResult, error) {
