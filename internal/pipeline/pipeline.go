...
type BracketOutcome struct {
	EntryIntent runtimeorder.Intent
	Entry       Result
	StopIntent  runtimeorder.Intent
	Stop        Result
}
...
type ReplaceOutcome struct {
	Request runtimeorder.ReplaceRequest
...
func (p *Pipeline) Evaluate(ctx context.Context, in Input) (Result, error) {
	...
	req, err := runtimeorder.NewRequest(planResult.Proposal, orderID)
	if err != nil {
...
func (p *Pipeline) submitBracket(ctx context.Context, in Input) (Result, error) {
	...
	entryIntent, err := p.buildBracketSubIntent(order.IntentEnter, validIntent, func(sub *runtimeorder.Intent) {
	...
	stopIntent, err := p.buildBracketSubIntent(order.IntentAdjustStop, validIntent, func(sub *runtimeorder.Intent) {
...
// sub-intent shares.
func (p *Pipeline) buildBracketSubIntent(kind order.IntentKind, bracket runtimeorder.Intent, set func(*runtimeorder.Intent)) (runtimeorder.Intent, error) {
	eventID, err := id.GenerateEventID(p.deps.IDs)
	if err != nil {
		return runtimeorder.Intent{}, fmt.Errorf("generating event id: %w", err)
	}
	intentID, err := id.GenerateIntentID(p.deps.IDs)
	if err != nil {
		return runtimeorder.Intent{}, fmt.Errorf("generating intent id: %w", err)
	}
	sub := runtimeorder.Intent{
		IntentID:   intentID,
		Kind:       kind,
