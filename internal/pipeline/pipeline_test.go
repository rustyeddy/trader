...
func mustEnterIntent(t *testing.T, ids *id.Generator, instID instrument.ID, side order.Side) runtimeorder.Intent {
	t.Helper()
...
func mustAdjustStopIntent(t *testing.T, ids *id.Generator, instID instrument.ID) runtimeorder.Intent {
	t.Helper()
	intentID, err := id.GenerateIntentID(ids)
	require.NoError(t, err)
	eventID, err := id.GenerateEventID(ids)
...
func TestPipelineSubmit_EndToEndAgainstSimulator(t *testing.T) {
	...
	assert.NotEqual(t, runtimeorder.Request{}, result.Request)
	assert.False(t, result.Request.OrderID.IsZero())
	assert.NotEqual(t, runtimeorder.Order{}, result.Order)
...
func TestPipelineSubmit_RiskRejectionNeverReachesBroker(t *testing.T) {
	...
	assert.NotEqual(t, runtimeorder.Proposal{}, result.Proposal, "Proposal is still populated on rejection")
	assert.Equal(t, runtimeorder.Request{}, result.Request, "Request must never be populated on rejection")
	assert.Equal(t, runtimeorder.Order{}, result.Order, "Order must never be populated on rejection")
...
func TestPipelineSubmit_InvalidIntentIsInvalidInput(t *testing.T) {
	...
	before := h.snapshot(t, ctx)
	_, err := p.Submit(ctx, pipeline.Input{
		Intent:  runtimeorder.Intent{}, // zero value: fails order.NewIntent
		Listing: h.listing,
		Account: before,
	})
...
func TestPipelineEvaluate_NeverCallsBroker(t *testing.T) {
	...
	assert.True(t, result.Decision.Allowed)
	assert.NotEqual(t, runtimeorder.Proposal{}, result.Proposal)
	assert.NotEqual(t, runtimeorder.Request{}, result.Request)
	assert.False(t, result.Request.OrderID.IsZero())
	assert.Equal(t, runtimeorder.Order{}, result.Order, "Evaluate must never populate Order")
}
...
func TestPipelineEvaluate_RejectionLeavesRequestZero(t *testing.T) {
	...
	assert.False(t, result.Decision.Allowed)
	assert.NotEqual(t, runtimeorder.Proposal{}, result.Proposal)
	assert.Equal(t, runtimeorder.Request{}, result.Request)
	assert.Equal(t, runtimeorder.Order{}, result.Order)
}
