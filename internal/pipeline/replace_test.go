...
func adjustStopIntentAt(t *testing.T, h testHarness, stopPrice string) runtimeorder.Intent {
	t.Helper()
	...
	rebuilt, err := runtimeorder.NewIntent(runtimeorder.Intent{
...
func TestPipelineSubmit_AdjustStop_InitialPlacement(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.Stop, result.Proposal.Type)
...
func TestPipelineSubmit_AdjustStop_RatchetsExistingStop(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.Proposal{}, result.Proposal, "the replace path never builds a new-order Proposal")
	...
	assert.NotEqual(t, runtimeorder.ReplaceResult{}, result.Replace.Result, "Submit must populate the broker's own Replace outcome")
