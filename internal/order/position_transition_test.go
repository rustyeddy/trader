...
func mustTransitionPosition(t *testing.T, side order.PositionSide, quantity, avgPrice string) Position {
	t.Helper()
...
func TestApplyFillToPositionOpensFromFlat(t *testing.T) {
	...
	result, err := ApplyFillToPosition(Position{}, false, accountID, listing, usdCode, order.Buy, num.MustParsePrice("1.10000"), num.MustParseQuantity("1000"))
	require.NoError(t, err)
	assert.Equal(t, order.Long, result.Position.Side)
...
func TestApplyFillToPositionOpensShortFromFlat(t *testing.T) {
	...
	result, err := ApplyFillToPosition(Position{}, false, accountID, listing, usdCode, order.Sell, num.MustParsePrice("1.10000"), num.MustParseQuantity("1000"))
	require.NoError(t, err)
	assert.Equal(t, order.Short, result.Position.Side)
	assert.True(t, result.RealizedPnL.IsZero())
	assert.Equal(t, TransitionOpen, result.Transition)
}
...
func TestApplyFillToPositionIncreasesLong(t *testing.T) {
	...
	accountID := mustAccountID(t)
	listing := mustEurUsdListing(t)
	existing := mustTransitionPosition(t, order.Long, "1000", "1.10000")

	result, err := ApplyFillToPosition(existing, true, accountID, listing, usdCode, order.Buy, num.MustParsePrice("1.09500"), num.MustParseQuantity("500"))
	require.NoError(t, err)
	assert.Equal(t, order.Long, result.Position.Side)
...
func TestApplyFillToPositionIncreasesShort(t *testing.T) {
	...
	accountID := mustAccountID(t)
	listing := mustEurUsdListing(t)
	existing := mustTransitionPosition(t, order.Short, "1000", "1.10000")

	result, err := ApplyFillToPosition(existing, true, accountID, listing, usdCode, order.Sell, num.MustParsePrice("1.10500"), num.MustParseQuantity("1000"))
	require.NoError(t, err)
	assert.Equal(t, order.Short, result.Position.Side)
...
func TestApplyFillToPositionReducesLongWithProfit(t *testing.T) {
	...
	// Sell 400 at 1.11000: a profitable partial close for a order.Long.
	result, err := ApplyFillToPosition(existing, true, accountID, listing, usdCode, order.Sell, num.MustParsePrice("1.11000"), num.MustParseQuantity("400"))
	require.NoError(t, err)
	assert.Equal(t, order.Long, result.Position.Side)
...
func TestApplyFillToPositionReducesShortWithLoss(t *testing.T) {
	...
	result, err := ApplyFillToPosition(existing, true, accountID, listing, usdCode, order.Buy, num.MustParsePrice("1.11000"), num.MustParseQuantity("400"))
	require.NoError(t, err)
	assert.Equal(t, order.Short, result.Position.Side)
...
func TestApplyFillToPositionClosesExactly(t *testing.T) {
	...
	result, err := ApplyFillToPosition(existing, true, accountID, listing, usdCode, order.Sell, num.MustParsePrice("1.12000"), num.MustParseQuantity("1000"))
	require.NoError(t, err)
	assert.Equal(t, order.Flat, result.Position.Side)
...
func TestApplyFillToPositionReverses(t *testing.T) {
	...
	// the fill price.
	result, err := ApplyFillToPosition(existing, true, accountID, listing, usdCode, order.Sell, num.MustParsePrice("1.11000"), num.MustParseQuantity("1500"))
	require.NoError(t, err)
	assert.Equal(t, order.Short, result.Position.Side)
