...
// silently combined.
func ApplyFillToPosition(existing Position, hasExisting bool, accountID id.AccountID, listing instrument.Listing, currency num.Currency, side order.Side, price num.Price, quantity num.Quantity) (PositionTransitionResult, error) {
...
func reduceOrReversePosition(existing Position, accountID id.AccountID, listing instrument.Listing, currency num.Currency, fillSide order.PositionSide, price num.Price, quantity num.Quantity) (PositionTransitionResult, error) {
	closedQty := quantity
	...
	case 0: // exact close
		pos, err := NewPosition(Position{AccountID: accountID, Listing: listing, Side: order.Flat})
