...
func checkOpenOrders(accountID id.AccountID, broker string, orders []runtimeorder.Order) ([]runtimeorder.Order, error) {
	seen := make(map[id.OrderID]struct{}, len(orders))
	cloned := make([]runtimeorder.Order, len(orders))
	for i, o := range orders {
		validated, err := runtimeorder.NewOrder(o)
		if err != nil {
			return nil, fmt.Errorf("entry %d: %w", i, err)
		}
...
// AvgPrice.
func clonePosition(p runtimeorder.Position) runtimeorder.Position {
	cloned := p
	if p.AvgPrice != nil {
		v := *p.AvgPrice
		cloned.AvgPrice = &v
...
func (s Snapshot) Positions() []runtimeorder.Position {
	cloned := make([]runtimeorder.Position, len(s.positions))
...
// does not affect s.
func (s Snapshot) OpenOrders() []runtimeorder.Order {
	cloned := make([]runtimeorder.Order, len(s.openOrders))
