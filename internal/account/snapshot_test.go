...
func validParams(t *testing.T) SnapshotParams {
	...
	return SnapshotParams{
		...
		Fees:            usd("12.34"),
		Financing:       usd("1.10"),
		Positions:       []runtimeorder.Position{mustPosition(t, accountID, listing)},
		OpenOrders:      []runtimeorder.Order{mustWorkingOrder(t, accountID, listing)},
	}
}
...
func TestSnapshotClonesEveryOrderPointerField(t *testing.T) {
	...
	p.OpenOrders = []runtimeorder.Order{o}
	...
	p.Positions = []runtimeorder.Position{mustPosition(t, p.AccountID, listing)}
...
func TestNewSnapshotRejectsPositionAccountMismatch(t *testing.T) {
	...
	p.Positions = []runtimeorder.Position{mustPosition(t, other, mustEurUsdListing(t))}
	_, err := NewSnapshot(p)
	require.ErrorIs(t, err, ErrInvalidSnapshot)
}
...
func TestNewSnapshotRejectsInvalidPosition(t *testing.T) {
	p := validParams(t)
	p.Positions = []runtimeorder.Position{{AccountID: p.AccountID}}
	_, err := NewSnapshot(p)
	require.ErrorIs(t, err, ErrInvalidSnapshot)
}
...
func TestNewSnapshotRejectsPositionProviderMismatch(t *testing.T) {
	...
	p.Positions = []runtimeorder.Position{mustPosition(t, p.AccountID, otherProviderListing)}
	_, err := NewSnapshot(p)
	require.ErrorIs(t, err, ErrInvalidSnapshot)
}
...
func TestNewSnapshotRejectsDuplicateListingPosition(t *testing.T) {
	p := validParams(t)
	listing := mustEurUsdListing(t)
	p.Positions = []runtimeorder.Position{
...
func TestNewSnapshotRejectsOpenOrderAccountMismatch(t *testing.T) {
	p := validParams(t)
	other := mustAccountID(t)
	p.OpenOrders = []runtimeorder.Order{mustWorkingOrder(t, other, mustEurUsdListing(t))}
	_, err := NewSnapshot(p)
	require.ErrorIs(t, err, ErrInvalidSnapshot)
}
...
func TestNewSnapshotRejectsOpenOrderProviderMismatch(t *testing.T) {
	...
	require.Equal(t, "OANDA", p.Broker)
	otherProviderListing := mustListing(t, "EUR", "USD", "IBKR", "EURUSD")
	p.OpenOrders = []runtimeorder.Order{mustWorkingOrder(t, p.AccountID, otherProviderListing)}
	_, err := NewSnapshot(p)
	require.ErrorIs(t, err, ErrInvalidSnapshot)
}
...
func TestNewSnapshotRejectsInvalidOpenOrder(t *testing.T) {
	p := validParams(t)
	p.OpenOrders = []runtimeorder.Order{{}}
	_, err := NewSnapshot(p)
	require.ErrorIs(t, err, ErrInvalidSnapshot)
}
...
func TestNewSnapshotRejectsTerminalOpenOrder(t *testing.T) {
	...
	p.OpenOrders = []runtimeorder.Order{filled}
	_, err := NewSnapshot(p)
	require.ErrorIs(t, err, ErrInvalidSnapshot)
}
...
func TestNewSnapshotRejectsDuplicateOrderID(t *testing.T) {
	...
	p.OpenOrders = []runtimeorder.Order{o, o}
	_, err := NewSnapshot(p)
	require.ErrorIs(t, err, ErrInvalidSnapshot)
}
