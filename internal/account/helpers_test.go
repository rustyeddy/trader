...
func mustPosition(t *testing.T, accountID id.AccountID, listing instrument.Listing) runtimeorder.Position {
	t.Helper()
...
func mustWorkingOrder(t *testing.T, accountID id.AccountID, listing instrument.Listing) runtimeorder.Order {
	t.Helper()
	...
	require.NoError(t, err)
	request, err := runtimeorder.NewRequest(proposal, mustOrderID(t))
	require.NoError(t, err)
...
func mustPartiallyFilledLimitOrder(t *testing.T, accountID id.AccountID, listing instrument.Listing) runtimeorder.Order {
	t.Helper()
	...
	require.NoError(t, err)
	partial, err := runtimeorder.ApplyFill(o, fill)
	require.NoError(t, err)
	return partial
}

func mustFilledOrder(t *testing.T, accountID id.AccountID, listing instrument.Listing) runtimeorder.Order {
	t.Helper()
