...
func TestNewEventRejectsInvalidOrder(t *testing.T) {
	e := baseEvent(t)
	e.Kind = broker.EventKindOrder
	invalid := runtimeorder.Order{} // fails order.NewOrder's validation
	e.Order = &invalid
...
func TestNewEventRejectsInvalidFill(t *testing.T) {
	e := baseEvent(t)
	e.Kind = broker.EventKindFill
	invalid := runtimeorder.Fill{} // fails order.NewFill's validation
	e.Fill = &invalid
...
// Account.Submit.
func mustSubmittedOrder(t *testing.T, accountID id.AccountID) runtimeorder.Order {
	t.Helper()
