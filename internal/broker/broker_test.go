...
func mustRequest(t *testing.T, accountID id.AccountID) runtimeorder.Request {
	t.Helper()
...
func TestAccountCancelTransitionsOrderToCanceled(t *testing.T) {
	...
	cancelReq, err := runtimeorder.NewCancelRequest(runtimeorder.CancelRequest{
		OrderID:  req.OrderID,
		Metadata: id.Metadata{EventID: mustEventID(t)},
	})
	require.NoError(t, err)
...
func TestAccountReplaceUpdatesAcceptedQuantity(t *testing.T) {
	...
	replaceReq, err := runtimeorder.NewReplaceRequest(runtimeorder.ReplaceRequest{
		OrderID:     req.OrderID,
		NewQuantity: &newQty,
		Metadata:    id.Metadata{EventID: mustEventID(t)},
	})
...
func TestEventsDeliversDeterministicOrderAcrossOperations(t *testing.T) {
	...
	assert.Equal(t, runtimeorder.StatusCanceled, events[1].Order.Status)
