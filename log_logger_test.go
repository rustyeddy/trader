...
func TestSetup_LevelDebug(t *testing.T) {
	require.NoError(t, tlog.Setup(tlog.LogConfig{Level: "debug"}))
}

func TestSetup_LevelWarn(t *testing.T) {
	require.NoError(t, tlog.Setup(tlog.LogConfig{Level: "warn"}))
}

func TestSetup_LevelError(t *testing.T) {
	require.NoError(t, tlog.Setup(tlog.LogConfig{Level: "error"}))
}

func TestSetup_LevelCaseInsensitive(t *testing.T) {
	for _, lvl := range []string{"DEBUG", "Info", "WARN", "ERROR", "warning"} {
		require.NoError(t, tlog.Setup(tlog.LogConfig{Level: lvl}), "level: %s", lvl)
	}
}
...
func TestPrewiredModuleAttr_Data(t *testing.T) {
	...
	require.NoError(t, tlog.Setup(tlog.LogConfig{Level: "debug", File: logPath}))
...
func TestSetup_ReInitialisation(t *testing.T) {
	...
	require.NoError(t, tlog.Setup(tlog.LogConfig{Level: "info", File: logPath1}))
	tlog.Info("first setup")

	require.NoError(t, tlog.Setup(tlog.LogConfig{Level: "debug", File: logPath2}))
...
func TestSetup_UnknownLevelDefaultsToInfo(t *testing.T) {
	...
	require.NoError(t, tlog.Setup(tlog.LogConfig{Level: "trace"}))
}
