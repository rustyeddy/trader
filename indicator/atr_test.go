...
func TestATR_ErrorGuards(t *testing.T) {
	_, err := NewATR(0, types.PriceScale)
	require.Error(t, err)
	_, err = NewATR(3, 0)
	require.Error(t, err)
}
...
func TestATR_WarmupAndReady(t *testing.T) {
	a, err := NewATR(3, types.PriceScale)
	...
	for i := 0; i < 3; i++ {
		a.Update(mkCandle(int32(types.PriceScale), 1.0, 1.01, 0.99, 1.0))
...
func TestATR_ValueIncreases_WithVolatility(t *testing.T) {
	a, err := NewATR(5, types.PriceScale)
	require.NoError(t, err)

	// seed with low-volatility candles
	for i := 0; i < 10; i++ {
		a.Update(mkCandle(int32(types.PriceScale), 1.0, 1.001, 0.999, 1.0))
	}
	require.True(t, a.Ready())
	lowATR := a.Float64()
	...
	for i := 0; i < 10; i++ {
		a.Update(mkCandle(int32(types.PriceScale), 1.0, 1.02, 0.98, 1.0))
	}
	highATR := a.Float64()
