...
func NewATR(period int, scale types.Scale6) (*ATR, error) {
	if period <= 0 {
...
func (a *ATR) PriceSum() types.PriceSum { return a.value }
func (a *ATR) Price() types.Price       { return types.Price(a.value) }
...
func (a *ATR) Update(c market.Candle) {
	...
	if a.periods < a.n {
		...
		a.sumTR += types.PriceSum(tr)
		a.value = types.PriceSum(roundDivPositive(int64(a.sumTR), int64(a.n)))
		a.ready = true
	} else {
		a.value = types.PriceSum(roundDivPositive(int64(a.value)*int64(a.n-1)+tr, int64(a.n)))
	}
