...
** Note: hour-bar alignment corrected by ADR-021

This ADR's own Decision section above states that "Minute and hour bars
... align to the UTC clock ... so H4 bars land on 00:00/04:00/.../20:00
UTC regardless of date." That specific claim was found, during M2's own
corpus validation (issue #81) against the real OANDA archive, to
disagree with OANDA's actual native H4 boundaries — an operational bug,
not merely a documentation gap: real H4 data was rejected outright (see
issue #99). *ADR-021* corrects this one sub-decision: hour bars (in
practice, only H4 — M1 and H1 are unaffected, see that ADR) now align to
the FX daily rollover instead of UTC midnight. Every other decision in
this ADR — the =Interval=/=TimeRange=/=Calendar= types, =FXCalendar='s
D1/W1 rollover alignment, DST handling via =time.Date=/=*time.Location=
— remains unchanged and in force. This note is additive, per this
registry's own rule against rewriting an accepted ADR's original
Decision text; the paragraph above is left exactly as originally
...
=time.Duration.Truncate= itself uses), not by truncating since the Unix
epoch.
...
types, =FXCalendar='s D1/W1 rollover alignment, DST handling). Per this
registry's own supersession rules, an accepted ADR's original text is
never rewritten; ADR-012 keeps its =Accepted= status and its Decision
section unchanged, with only a small additive forward-pointer note
appended directing readers to this ADR for the corrected sub-decision.
Marking the entire ADR-012 =Superseded= was considered and rejected — see
Alternatives Considered.
...
- *Mark all of ADR-012 =Superseded=.* Rejected: ADR-012 bundles several
  ...
  those parts were also being replaced. A narrow correcting ADR plus an
  additive forward-pointer note keeps the historical record accurate
  without overclaiming scope.
