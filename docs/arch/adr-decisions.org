...
| 022 | CLI and application service layer (in adr-022-cli-app-service-layer.org) | Proposed |
...
column above is kept in sync with it.  It remains =Proposed=: issue
#104 settled the application package name (=service=) while building
the service-layer foundation, but four open questions (whether
=Update= grows policy/options; where CLI formatting lives; default
=--to= behavior; confirmation/error UX for read versus mutation
commands) remain unresolved, and per =CONTRIBUTING.org= should not be
treated as binding until those are settled or explicitly deferred.
