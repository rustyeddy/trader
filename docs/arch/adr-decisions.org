...
| 020 | Historic data subsystem (in adr-020-historic-data.org)            | See file   |

Note: ADR-020 is maintained in its own file,
=docs/arch/adr-020-historic-data.org=, rather than inline in this
registry.  Consult that file for its current status, decisions, and open
questions; the row above is a pointer so the registry index remains
complete.
