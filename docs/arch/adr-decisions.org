...
=docs/arch/adr-022-cli-app-service-layer.md=, rather than inline in
this registry.  Its own Status section is authoritative; the status
column above is kept in sync with it.  It remains =Proposed=: it has
five open questions (application package name; whether =Update= grows
policy/options; where CLI formatting lives; default =--to= behavior;
confirmation/error UX for read versus mutation commands) still
unresolved, and per =CONTRIBUTING.org= should not be treated as
binding until those are settled or explicitly deferred.
